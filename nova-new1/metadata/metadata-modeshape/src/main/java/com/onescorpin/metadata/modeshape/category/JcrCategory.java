...
import com.onescorpin.metadata.modeshape.category.security.JcrCategoryAllowedActions;
...
public class JcrCategory extends AbstractJcrAuditableSystemEntity implements Category, AccessControlledMixin {
    ...
    // a it can be passed to each nflow entity when they are constructed.
    ...
    public JcrCategory(Node node, NflowOpsAccessControlProvider opsAccessProvider) {
        super(node);
    ...
    public void enableAccessControl(JcrAllowedActions prototype, Principal owner, List<SecurityRole> catRoles, List<SecurityRole> nflowRoles) {
        // Setup default access control for this entity
        AccessControlledMixin.super.enableAccessControl(prototype, owner, catRoles);
        
        // Setup the nflow roles relationships.
        getDetails().ifPresent(d -> d.enableNflowRoles(nflowRoles));
    }
    ...
    public List<? extends Nflow> getNflows() {
        return getDetails().map(d -> d.getNflows()).orElse(Collections.emptyList());
    }
    ...
    public Set<RoleMembership> getNflowRoleMemberships() {
        return getDetails().map(d -> d.getNflowRoleMemberships()).orElse(null);
    }
    
    @Override
    public Optional<RoleMembership> getNflowRoleMembership(String roleName) {
        return getDetails().map(d -> d.getNflowRoleMembership(roleName)).orElse(null);
    }
    ...
    public String getNflowParentPath() {
