package com.onescorpin.metadata.modeshape.template.security;
...
public class JcrTemplateAllowedActions extends JcrAllowedActions {
    ...
    private JcrNflowTemplate template;
    ...
    public JcrTemplateAllowedActions(Node allowedActionsNode) {
        ...
        this.template = JcrUtil.getJcrObject(JcrUtil.getParent(allowedActionsNode), JcrNflowTemplate.class);
    }
    ...
    public void setupAccessControl(Principal owner) {
        ...
        //this is needed since when a nflow is created it needs to set the template bi-directional relationship.
        //Nova will handle the explicitly permission checks to modify a template using its permissions rather than JCR privileges.
