...
public class JcrCategoryAllowedActions extends JcrAllowedActions {
    ...
    protected void updateEntityAccess(Principal principal, Set<? extends Action> actions) {
        ...
        actions.forEach(action -> {
            ...
            } else if (action.implies(CategoryAccessControl.CREATE_NFLOW)) {
                // Privilege.JCR_MODIFY_ACCESS_CONTROL is needed here since anyone creating a new nflow will inherit the ACL access from this parent 
                // category details node before the access control is update in the new nflow node, and the new nflow owner needs rights to change
                // the access control of the nflow it is creating.  TODO: Perhaps we should refactor in a future release to create a simple child node 
                // that the nflow nodes attach to so that that node only can have Privilege.JCR_MODIFY_ACCESS_CONTROL for the user
