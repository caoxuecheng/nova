...
/**
 * Check the Version of Nova looking at the version.txt file found in the /conf folder of the Nova install
 */
public class NovaVersionUtil {

    private static final Logger log = LoggerFactory.getLogger(NovaVersionUtil.class);

    public static NovaVersion getBuildVersion() {
    ...
    public static boolean isUpToDate(NovaVersion dbVersion) {
        NovaVersion deployedVersion = getBuildVersion();
        return dbVersion != null && deployedVersion != null && deployedVersion.equals(dbVersion);
    }

    public static NovaVersion parseVersion(String versionString) {
    ...
    private static String getBuildVersionString() {
        if (StringUtils.isBlank(versionString)) {
            ...
            InputStream in = NovaVersionUtil.class.getClassLoader()
                .getResourceAsStream(versionFile);
            URL url = NovaVersionUtil.class.getClassLoader().getResource(versionFile);
            if (in != null) {
                try {
                    try {
                    ...
                    log.info("loaded Nova version file: {}  build Time: {}", currentVersion, buildTimestamp);
                } catch (IOException e) {
    ...
    public static class Version implements NovaVersion {
        ...
        public Version(NovaVersion version) {
