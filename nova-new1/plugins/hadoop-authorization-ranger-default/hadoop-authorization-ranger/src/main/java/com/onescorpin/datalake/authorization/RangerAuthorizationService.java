package com.onescorpin.datalake.authorization;

/*-
 * #%L
 * onescorpin-hadoop-authorization-ranger
 * %%
...
import com.onescorpin.datalake.authorization.config.AuthorizationConfiguration;
import com.onescorpin.datalake.authorization.config.RangerConnection;
import com.onescorpin.datalake.authorization.model.HadoopAuthorizationGroup;
import com.onescorpin.datalake.authorization.rest.client.RangerRestClient;
import com.onescorpin.datalake.authorization.rest.client.RangerRestClientConfig;
import com.onescorpin.datalake.authorization.rest.model.RangerCreateOrUpdatePolicy;
import com.onescorpin.datalake.authorization.rest.model.RangerGroup;
import com.onescorpin.datalake.authorization.rest.model.RangerPolicy;
import com.onescorpin.datalake.authorization.service.BaseHadoopAuthorizationService;
...
public class RangerAuthorizationService extends BaseHadoopAuthorizationService {
    ...
    private static final String NOVA_POLICY_PREFIX = "nova";
    ...
    @Override
    public void createOrUpdateReadOnlyHivePolicy(String categoryName, String nflowName, List<String> securityGroupNames, String datebaseName, List<String> tableNames) {
        String rangerHivePolicyName = getHivePolicyName(categoryName, nflowName);
        ...
        if (hadoopPolicyList.size() > 1) {
            ...
            createReadOnlyHivePolicy(categoryName, nflowName, securityGroupNames, datebaseName, tableNames);
        } else {
            updateReadOnlyHivePolicy(categoryName, nflowName, securityGroupNames, datebaseName, tableNames);
        }
    }

    @Override
    public void createOrUpdateReadOnlyHdfsPolicy(String categoryName, String nflowName, List<String> securityGroupNames, List<String> hdfsPaths) {
        String rangerHdfsPolicyName = getHdfsPolicyName(categoryName, nflowName);
        ...
        if (hadoopPolicyList.size() > 1) {
            ...
            createReadOnlyHdfsPolicy(categoryName, nflowName, securityGroupNames, hdfsPaths);
        } else {
            updateReadOnlyHdfsPolicy(categoryName, nflowName, securityGroupNames, hdfsPaths);
        }
    }

    @Override
    public void createReadOnlyHivePolicy(String categoryName, String nflowName, List<String> securityGroupNames, String databaseName, List<String> tableNames) {
    ...
    @Override
    public void createReadOnlyHdfsPolicy(String categoryName, String nflowName, List<String> securityGroupNames, List<String> hdfsPaths) {
    ...
    @Override
    public void updateReadOnlyHivePolicy(String categoryName, String nflowName, List<String> groups, String datebaseNames, List<String> tableNames) {
    ...
    @Override
    public void updateReadOnlyHdfsPolicy(String categoryName, String nflowName, List<String> groups, List<String> hdfsPaths) {
     ...
     */
    public void updateSecurityGroupsForAllPolicies(String categoryName, String nflowName, List<String> securityGroupNames, Map<String, Object> nflowProperties) {
        String rangerHdfsPolicyName = getHdfsPolicyName(categoryName, nflowName);
        ...
        if (securityGroupNames == null || securityGroupNames.isEmpty()) {
            // Only delete if the policies exists. It's possibile that someone adds a security group right after nflow creation and before initial ingestion
            if (hivePolicy != null) {
                deleteHivePolicy(categoryName, nflowName);
            }
            if (hdfsPolicy != null) {
                deleteHdfsPolicy(categoryName, nflowName, null);
            }
        } else {
            if (hdfsPolicy == null) {
                // If a nflow hasn't run yet the metadata won't exists
                if (!StringUtils.isEmpty((String) nflowProperties.get(REGISTRATION_HDFS_FOLDERS))) {
                    String hdfsFoldersWithCommas = ((String) nflowProperties.get(REGISTRATION_HDFS_FOLDERS)).replace("\n", ",");
                    List<String> hdfsFolders = Stream.of(hdfsFoldersWithCommas).collect(Collectors.toList());
                    createReadOnlyHdfsPolicy(categoryName, nflowName, securityGroupNames, hdfsFolders);
                }
            } else {
                List<String> hdfsPermissions = new ArrayList<>();
                hdfsPermissions.add(HDFS_READ_ONLY_PERMISSION);
            ...
            if (hivePolicy == null) {
                // If a nflow hasn't run yet the metadata won't exists
                if (!StringUtils.isEmpty((String) nflowProperties.get(REGISTRATION_HIVE_TABLES))) {
                    String hiveTablesWithCommas = ((String) nflowProperties.get(REGISTRATION_HIVE_TABLES)).replace("\n", ",");
                    List<String> hiveTables = Stream.of(hiveTablesWithCommas).collect(Collectors.toList());
                    String hiveSchema = ((String) nflowProperties.get(REGISTRATION_HIVE_SCHEMA));
                    createOrUpdateReadOnlyHivePolicy(categoryName, nflowName, securityGroupNames, hiveSchema, hiveTables);
                }
            } else {
                List<String> hivePermissions = new ArrayList<>();
                hivePermissions.add(HIVE_READ_ONLY_PERMISSION);
    ...
    private String getHdfsPolicyName(String categoryName, String nflowName) {
        return NOVA_POLICY_PREFIX + "_" + categoryName + "_" + nflowName + "_" + HDFS_REPOSITORY_TYPE;
    }

    private String getHivePolicyName(String categoryName, String nflowName) {
        return NOVA_POLICY_PREFIX + "_" + categoryName + "_" + nflowName + "_" + HIVE_REPOSITORY_TYPE;
    }
    ...
    @Override
    public void deleteHivePolicy(String categoryName, String nflowName) {
        String rangerHivePolicyName = getHivePolicyName(categoryName, nflowName);
    ...
    @Override
    public void deleteHdfsPolicy(String categoryName, String nflowName, List<String> hdfsPaths) {
