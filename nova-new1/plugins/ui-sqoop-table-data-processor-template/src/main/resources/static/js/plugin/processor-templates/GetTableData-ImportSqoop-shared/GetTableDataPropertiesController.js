...
define(['angular'], function (angular) {
    ...
    var controller =  function($scope, $q,$http,$mdToast,RestUrlService, NflowService,EditNflowNifiPropertiesService,DBCPTableSchemaService) {
        ...
        /**
         * If we are creating a new nflow then use the Create Model, otherwise use the Edit Model
         * If the processor is undefined, attempt to get it via the inputProcessor of the model
         */
        if(this.mode == 'create') {
            if (this.processor == undefined) {
                this.processor = NflowService.createNflowModel.inputProcessor;
            }
            ...
            if (this.processor == undefined) {
                this.processor = EditNflowNifiPropertiesService.editNflowModel.inputProcessor;
            }
            this.model = EditNflowNifiPropertiesService.editNflowModel;
         ...
         *
         * Currently, this handles the output type property specified by the user for a database ingest nflow
            ...
            if (self.mode == 'create') {
                if (nifiPropertyChanged.key == 'Output Type') {
                    if (nifiPropertyChanged.value == 'AVRO') {
                        self.model.table.nflowFormat = 'STORED AS AVRO'
                    }
                    else if (nifiPropertyChanged.value == 'DELIMITED') {
                        self.model.table.nflowFormat = NflowService.getNewCreateNflowModel().table.nflowFormat
                    }
                }
            }
        };
       ...
       function queryTablesSearch (query) {
        ...
        /**
         * Callback from saving/edit nflow
         */
        ...
        function describeTable(){
            ...
            if(dbcpProperty != null && dbcpProperty.value != null && self.selectedTable != null) {
                var successFn = function (response) {
                    ...
                    NflowService.setTableFields(self.tableSchema.fields);
        ...
        this.onManualFieldNameChange = function () {
            ...
            _.each(val.split(","), function (field) {
                var col = NflowService.newTableFieldDefinition();
                col.name = field.trim();
            ...
            NflowService.setTableFields(fields);
        ...
        this.init = function() {
            ...
            function setupClonedNflowTableFields(){
                self.databaseConnectionError = false;
            ...
            /**
             * if we are editing or working with the cloned nflow then get the selectedTable saved on this model.
             */
            if(self.mode == 'edit'){
             ...
             //if we are cloning and creating a new nflow setup the autocomplete
                initializeAutoComplete();
                setupClonedNflowTableFields();
            }
                ...
                if(tableProperty && newVal != undefined) {
                    ...
                    if (newVal != null && newVal != undefined) {
                        if(self.mode == 'create') {
                            ...
                            //If we are working with a cloned nflow we should attempt to get the field information from the cloned model
    ...
    var moduleName = "nova.plugin.processor-template.tabledata";
    angular.module(moduleName, [])
    angular.module(moduleName).controller('GetTableDataPropertiesController',["$scope","$q","$http","$mdToast","RestUrlService","NflowService","EditNflowNifiPropertiesService","DBCPTableSchemaService", controller]);

    angular.module(moduleName)
        .directive('onescorpinGetTableDataProperties', directive);
