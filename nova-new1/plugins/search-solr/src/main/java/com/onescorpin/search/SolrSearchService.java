...
import com.onescorpin.search.transform.SolrSearchResultTransform;
...
public class SolrSearchService implements Search {
    ...
    public void delete(@Nonnull final String indexName, @Nonnull final String typeName, @Nonnull final String id, @Nonnull final String schema, @Nonnull final String table) {
        buildRestClient();
        final String dataCollectionName = "nova-data";
        try {
            //deletes the schema
        ...
        try {
            //delete the data
            client.deleteByQuery(dataCollectionName,"nova_schema:" + schema + " AND nova_table:" + table);
    ...
    private QueryResponse executeSearch(String query, int size, int start) {
        final String COLLECTION_LIST = "nova-data," + SearchIndex.DATASOURCES;    //Solr admin to configure beforehand
        ...
        try {
            return client.query("nova-data", solrQuery);
