...
#################################################################################################
#	Run this script before starting Nova services for the 1st time, for Elasticsearch (v2,v5)   #
#################################################################################################
...
if [ $# -eq 4 ]
then
	echo "Setting up nova indexes in Elasticsearch: host="$ELASTIC_SEARCH_HOST", port="$ELASTIC_SEARCH_REST_PORT", num-shards="$NUMBER_OF_SHARDS", num-replicas="$NUMBER_OF_REPLICAS
else
...
do
	curl -XPUT $ELASTIC_SEARCH_HOST:$ELASTIC_SEARCH_REST_PORT/$NOVA_INDEX?pretty -H 'Content-Type: application/json' -d"$BODY"
done
