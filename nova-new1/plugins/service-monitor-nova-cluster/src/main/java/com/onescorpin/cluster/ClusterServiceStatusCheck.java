...
/*-
 * #%L
 * nova-service-monitor-nova-cluster
 * %%
...
/**
 * Monitor the status of the nova cluster
 */
public class ClusterServiceStatusCheck implements ServiceStatusCheck {

    @Value("${nova.cluster.nodeCount:#{null}}")
    ...
    public ServiceStatusResponse healthCheck() {
        ...
        String serviceName = "Nova Cluster";
        ...
        if(isClustered) {
            ...
            if(StringUtils.isNotBlank(expectedNodes)) {
                try {
                    ...
                    valid = false;
                    serviceMessage = "  Unable to validate the expected nova node count.  Ensure the 'nova.cluster.nodeCount' property is set to a valid number.";
                }
            }
            ...
            serviceMessage ="Nova is not configured to be running as a cluster";
            ServiceComponent component =
