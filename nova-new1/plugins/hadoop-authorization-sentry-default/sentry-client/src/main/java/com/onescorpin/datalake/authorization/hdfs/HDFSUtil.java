package com.onescorpin.datalake.authorization.hdfs;
...
public class HDFSUtil {
    ...
    /**
     * @param allPathForAclCreation : Each Nova internal path
    ...
    public void splitPathListAndApplyPolicy(String allPathForAclCreation, Configuration conf, FileSystem fileSystem, String groupList, String hdfs_permission) throws IOException {
        String []allNovaIntermediatePath = allPathForAclCreation.split(",");
        for (int pathCounter = 0; pathCounter < allNovaIntermediatePath.length; pathCounter++) {
            try {
                individualIntermediatePathApplyPolicy(conf, fileSystem, allNovaIntermediatePath[pathCounter], groupList, hdfs_permission);
            } catch (IOException e) {
    ...
    public void splitPathListAndFlushPolicy(String allPathForAclCreation, Configuration conf, FileSystem fileSystem) throws IOException {
        ...
        for (int pathCounter = 0; pathCounter < allNovaIntermediatePath.length; pathCounter++) {
            try {
                individualIntermediatePathFlushPolicy(conf, fileSystem, allNovaIntermediatePath[pathCounter]);
            } catch (IOException e) {
                log.error("Unable to iterate on HDFS directories " + e.getMessage());
                throw new IOException(e);
            }
    ...
    public void individualIntermediatePathApplyPolicy(Configuration conf, FileSystem fileSystem, String novaPath, String groupList, String hdfs_permission) throws IOException {
        Path path = new Path(novaPath);
        fileSystem = path.getFileSystem(conf);
        listAllDirAndApplyPolicy(fileSystem, path, groupList, hdfs_permission);
    }

    public void individualIntermediatePathFlushPolicy(Configuration conf, FileSystem fileSystem, String novaPath) throws IOException {
        Path path = new Path(novaPath);
        fileSystem = path.getFileSystem(conf);
