define(['angular', "search/module-name"], function (angular, moduleName) {
    ...
    var controller = function ($scope, $sce, $http, $mdToast, $mdDialog, $transition$, SearchService, Utils, CategoriesService, StateService, NflowService,PaginationDataService) {
        ...
        this.onSearchResultItemClick = function ($event, result) {
            ...
            switch (result.type) {
                ...
                case "NOVA_NFLOWS":
                    ...
                    CategoriesService.getCategoryById(this.cleanValue(result.nflowCategoryId)).then(function (category1) {
                        category = category1;
                        NflowService.getNflowByName(category.systemName + "." + (result.nflowSystemName.replace('[', '').replace(']', ''))).then(function (nflow1) {
                            nflow = nflow1;
    ...
    angular.module(moduleName).controller('SearchController',
        ["$scope", "$sce", "$http", "$mdToast", "$mdDialog", "$transition$", "SearchService", "Utils", "CategoriesService", "StateService", "NflowService","PaginationDataService", controller]);
});
