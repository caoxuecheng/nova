define(['angular','nflow-mgr/module-name'], function (angular,moduleName) {
    angular.module(moduleName).service('DBCPTableSchemaService', ["RestUrlService", function (RestUrlService) {
        ...
        this.LIST_TABLES_URL = function (serviceId) {
            return this.ROOT + "/proxy/v1/nflowmgr/nifi/controller-services/" + serviceId + "/tables";
        };

        this.DESCRIBE_TABLE_URL = function (serviceId, tableName) {
            return this.ROOT + "/proxy/v1/nflowmgr/nifi/controller-services/" + serviceId + "/tables/" + tableName;
        };
    }]);
});
