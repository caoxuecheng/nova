...
define(['angular','nflow-mgr/module-name'], function (angular,moduleName) {
    angular.module(moduleName).factory('RegisterTemplateService', ["$http","$q","$mdDialog","RestUrlService","NflowInputProcessorOptionsFactory","NflowDetailsProcessorRenderingHelper","NflowPropertyService","AccessControlService","EntityAccessControlService",function ($http, $q, $mdDialog, RestUrlService, NflowInputProcessorOptionsFactory, NflowDetailsProcessorRenderingHelper,NflowPropertyService,AccessControlService,EntityAccessControlService) {
        ...
        /**
         * for a given processor assign the nflowPropertiesUrl so it can be rendered correctly
         * @param processor
         */
        function setRenderTemplateForProcessor(processor, mode) {
            if (processor.nflowPropertiesUrl == undefined) {
                processor.nflowPropertiesUrl = null;
            }
            if (processor.nflowPropertiesUrl == null) {
                NflowInputProcessorOptionsFactory.setNflowProcessingTemplateUrl(processor, mode);
            }
        }
        ...
        var data = {
             ...
             */
            customPropertyRendering: ["metadata.table.targetFormat", "metadata.table.nflowFormat"],
            ...
            emptyModel: {
                ...
                ports: [],
                reusableTemplateConnections: [],  //[{reusableTemplateNflowName:'', nflowOutputPortName: '', reusableTemplateInputPortName: ''}]
            ...
            newReusableConnectionInfo: function () {
                return [{reusableTemplateNflowName: '', nflowOutputPortName: '', reusableTemplateInputPortName: ''}];
            },
            ...
            fetchRegisteredReusableNflowInputPorts: function () {

                var successFn = function (response) {
                    self.nflowInputPortMap = response.data;
                }
                var errorFn = function (err) {

                }
                var promise = $http.get(RestUrlService.ALL_REUSABLE_NFLOW_INPUT_PORTS);
            ...
            /**
             * Nflow Processors can setup separate Templates to have special rendering done for a processors properties.
             * @see /js/define-nflow/nflow-details/get-table-data-properties.
             *  This
             * @param key
             * @returns {boolean}
             */
            ...
            removeNonUserEditableProperties: function (processorArray, keepProcessorIfEmpty) {
                ...
                _.each(processorArray, function (processor, i) {
                    ...
                    if (NflowDetailsProcessorRenderingHelper.isGetTableDataProcessor(processor) || NflowDetailsProcessorRenderingHelper.isWatermarkProcessor(processor)) {
            ...
            /**
             * Updates the nflowProcessingTemplateUrl for each processor in the model
             * @param model
             */
             ...
             * used in Nflow creation and nflow details to render the nifi input fields
             * @param template
             */
            initializeProperties: function (template, mode, nflowProperties) {
                //get the saved properties

                var savedProperties = {};
                if (nflowProperties) {
                    _.each(nflowProperties, function (property) {
                ...
                function updateProperties(processor, properties) {
                    ...
                    _.each(properties, function (property) {
                        ...
                        NflowPropertyService.initSensitivePropertyForEditing(property);

                        NflowPropertyService.updateDisplayValue(property);
            ...
            loadTemplateWithProperties: function (registeredTemplateId, nifiTemplateId, templateName) {
                ...
                if (self.model.nifiTemplateId != null) {
                    ...
                    var successFn = function (response) {
                        ...
                        self.model.nflowsCount = templateData.nflowsCount;
