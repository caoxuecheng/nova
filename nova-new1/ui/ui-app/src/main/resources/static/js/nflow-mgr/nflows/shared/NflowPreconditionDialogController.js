...
define(['angular','nflow-mgr/nflows/module-name'], function (angular,moduleName) {
    ...
    var controller = function($scope, $mdDialog, $mdToast, $http, StateService, NflowService, PolicyInputFormService, nflow, index) {
        $scope.nflow = nflow;
        $scope.options = [];

        $scope.ruleMode = 'NEW';

        NflowService.getPossibleNflowPreconditions().then(function(response) {
            ...
            $scope.options = PolicyInputFormService.groupPolicyOptions(response.data, currentNflowValue);
            ruleTypesAvailable();
        });

        var arr = nflow.schedule.preconditions;
        ...
        $scope.deletePrecondition = function() {
            ...
            nflow.schedule.preconditions = $scope.preconditions;
    ...
    angular.module(moduleName).controller("NflowPreconditionsDialogController", ["$scope","$mdDialog","$mdToast","$http","StateService","NflowService","PolicyInputFormService","nflow","index",controller]);
