...
export class TransformDataComponent implements OnInit {
    ...
    constructor(private $scope: angular.IScope, $element: angular.IAugmentedJQuery, private $q: angular.IQService, private $mdDialog: angular.material.IDialogService,
                private domainTypesService: DomainTypesService, private RestUrlService: any, SideNavService: any, private uiGridConstants: any, private NflowService: any, private BroadcastService: any,
     ...
     */
    private saveToNflowModel() {
        // Add unsaved filters
        this.addFilters();

        // Check if updates are necessary
        let nflowModel = this.NflowService.createNflowModel;
        let newScript = this.engine.getNflowScript();
        ...
        if (fields !== null) {
            ...
            this.query().then(() => {
                this.NflowService.setTableFields(this.engine.getFields(), this.engine.getFieldPolicies());
                this.NflowService.syncTableFieldPolicyNames();
                this.engine.save();
                deferred.resolve(true);
            });
        ...
        if (domainType) {
            ...
            this.fieldPolicies = this.engine.getColumns().map((column, index) => {
                ...
                if (index < fieldPolicies.length) {
                    ...
                    fieldPolicy = this.NflowService.newTableFieldPolicy(column.hiveColumnLabel);
                    fieldPolicy.fieldName = column.hiveColumnLabel;
                    fieldPolicy.nflowFieldName = column.hiveColumnLabel;
                }

                if (index === columnIndex) {
                    this.NflowService.setDomainTypeForField({}, fieldPolicy, domainType);
                }
