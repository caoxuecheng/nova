    ...
    var controller = function ($scope,$element,$http, $q,$interval,StateService,OpsManagerJobService,OpsManagerDashboardService, HttpService,ChartJobStatusService,BroadcastService) {
        ...
        function updateCounts(responseData) {
            ...
            if(responseData){
                ...
                ensureNflowSummaryMatches(responseData);
            }
        }

        /**
         * Job Status Counts run every second.
         * Nflow Healh/Running data runs every 5 seconds.
         * if the Job Status changes, update the summary data and notify the Nflow Health Card
         * @param jobStatus the list of Job Status counts by nflow
         */
        function ensureNflowSummaryMatches(jobStatus) {
            var summaryData = OpsManagerDashboardService.nflowSummaryData;
            var nflowSummaryUpdated = [];
            var runningNflowNames = [];
            var notify = false;
            _.each(jobStatus, function (nflowJobStatusCounts) {
                var nflowSummary = summaryData[nflowJobStatusCounts.nflowName];
                if (angular.isDefined(nflowSummary)) {
                    var summaryState = nflowSummary.state;
                    if (nflowJobStatusCounts.status == "RUNNING") {
                        runningNflowNames.push(nflowJobStatusCounts.nflowName);
                    }
                    if (nflowJobStatusCounts.status == "RUNNING" && summaryState != "RUNNING") {
                        //set it
                        nflowSummary.state = "RUNNING"
                        nflowSummary.runningCount = summaryData.count
                        //trigger update of nflow summary
                        nflowSummaryUpdated.push(nflowSummary);
                        notify = true;
                    }
                }
            });
            //any of those that are not in the runningNflowNames are not running anymore
            var notRunning = _.difference(Object.keys(summaryData), runningNflowNames);
            _.each(notRunning, function (nflowName) {
                var summary = summaryData[nflowName];
                if (summary && summary.state == "RUNNING") {
                    summary.state = "WAITING";
                    summary.runningCount = 0;
                    nflowSummaryUpdated.push(summary);
                    notify = true;
                }
            })

            if ( notify == true) {
                BroadcastService.notify(OpsManagerDashboardService.NFLOW_SUMMARY_UPDATED, nflowSummaryUpdated);
            }
        }
