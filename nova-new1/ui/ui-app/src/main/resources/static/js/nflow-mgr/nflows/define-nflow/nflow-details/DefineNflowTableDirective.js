...
define(['angular','nflow-mgr/nflows/define-nflow/module-name'], function (angular,moduleName) {
    ...
    var directive = function () {
        return {
            ...
            scope: {},
            require: ['onescorpinDefineNflowTable', '^onescorpinStepper'],
            controllerAs: 'vm',
            templateUrl: 'js/nflow-mgr/nflows/define-nflow/nflow-details/define-nflow-table.html',
            controller: "DefineNflowTableController",
    ...
    var controller = function ($rootScope, $scope, $http, $timeout, $mdToast, $filter, $mdDialog, $mdExpansionPanel, RestUrlService, NflowService, FileUpload, BroadcastService, Utils, NflowTagService,
                               DomainTypesService) {

        this.defineNflowTableForm = {};
        var self = this;
        ...
        this.availableDefinitionDataTypes = NflowService.columnDefinitionDataTypes.slice();
        this.schemaParser = {};
         ...
         */
        this.syncNflowsColumns = function() {

            _.each(self.model.table.tableSchema.fields, function (columnDef) {
                initNflowColumn(columnDef);
            });
        }

        function initNflowColumn(columnDef){
        ...
        function newColumnDefinition() {
            return NflowService.newTableFieldDefinition();
        }
        ...
        function newColumnPolicy() {
            return NflowService.newTableFieldPolicy();
        }
        ...
        function fieldNamesUnique(resetFormReadyCheck) {
            ...
            var formReady = _.every(self.model.table.tableSchema.fields, function(columnDef) {
                ...
                if (!angular.isDefined(self.defineNflowTableForm["name_" + columnDef._id])) {
                    return false;
                }

                if (columnDef.deleted === true) {
                    self.defineNflowTableForm["name_" + columnDef._id].$setValidity("required", true);
                    self.defineNflowTableForm["datatype_" + columnDef._id].$setValidity("required", true);
                }

                // Check for reserved names
                if (columnDef.name === "processing_dttm") {
                    self.defineNflowTableForm["name_" + columnDef._id].$setValidity("reserved", false);
                } else {
                    self.defineNflowTableForm["name_" + columnDef._id].$setValidity("reserved", true);
                }

                // Check for duplicate column names
                if (angular.isDefined(notUnique[columnDef.name])) {
                    self.defineNflowTableForm["name_" + columnDef._id].$setValidity("notUnique", false);
                } else {
                    self.defineNflowTableForm["name_" + columnDef._id].$setValidity("notUnique", true);
                }
            ...
            _.chain(self.model.table.partitions).groupBy(function (partition) {
                ...
                if (group.length > 1) {
                    _.each(group, function (partition) {
                        ...
                        self.defineNflowTableForm['partition_name' + partition._id].$setValidity('notUnique', false);
                    });
                }
                else {
                    _.each(group, function (partition) {
                        //valid this is a unique partition name
                        self.defineNflowTableForm['partition_name' + partition._id].$setValidity('notUnique', true);
                    });
                }
            });
        ...
        this.addColumn = function (columnDef, syncFieldPolicies) {
            ...
            if (columnDef.sampleValues != null && columnDef.sampleValues.length > 0) {
                ...
                if (domainType !== null) {
                    NflowService.setDomainTypeForField(columnDef, policy, domainType);
                }
            } else {
            ...
            if (syncFieldPolicies == undefined || syncFieldPolicies == true) {
                NflowService.syncTableFieldPolicyNames();
            }
        };
        ...
        function validate(validForm) {
            if (validForm == undefined) {
                validForm = self.defineNflowTableForm.$valid ;
            }
        ...
        //Set the Table Name to be the System Nflow Name
        var systemNflowNameWatch = $scope.$watch(function () {
            return self.model.systemNflowName;
        }, function (newVal) {
        ...
        var formValidWatch = $scope.$watch(function () {
            return self.defineNflowTableForm.$valid;
        }, function (newVal) {
            if (newVal == true) {
                validate(newVal);
            }
        ...
        this.uploadSampleFile = function () {
            ...
            var successFn = function (response) {
                ...
                resetColumns();
                self.availableDefinitionDataTypes = NflowService.columnDefinitionDataTypes.slice();
                ...
                NflowService.syncTableFieldPolicyNames();
                //set the nflowFormat property
                self.model.table.nflowFormat = responseData.hiveFormat;
        ...
        function touchErrorFields() {
            var errors = self.defineNflowTableForm.$error;
        ...
        NflowService.getPartitionFunctions()
        ...
        $scope.$on('$destroy', function () {
            systemNflowNameWatch();
        });
    };

    angular.module(moduleName).controller('DefineNflowTableController', ["$rootScope","$scope","$http","$timeout","$mdToast","$filter","$mdDialog","$mdExpansionPanel","RestUrlService","NflowService","FileUpload","BroadcastService","Utils", "NflowTagService", "DomainTypesService", controller]);

    angular.module(moduleName)
        .directive('onescorpinDefineNflowTable', directive);

    angular.module(moduleName).filter("filterPartitionFormula", ["NflowService", function(NflowService) {
        return function(formulas, partition) {
            // Find column definition
            var columnDef = (partition && partition.sourceField) ? NflowService.getColumnDefinitionByName(partition.sourceField) : null;
            if (columnDef == null) {
                return formulas;
            }
