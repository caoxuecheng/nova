    ...
    angular.module(moduleName).factory('SlaEmailTemplateService', ["$http","$q","$mdToast","$mdDialog","RestUrlService",function ($http, $q, $mdToast, $mdDialog, RestUrlService) {
        ...
        var data = {
            ...
            getExistingTemplates: function () {
                var promise = $http.get("/proxy/v1/nflowmgr/sla/email-template");
            ...
            getRelatedSlas: function(id) {
               return $http.get("/proxy/v1/nflowmgr/sla/email-template-sla-references",{params:{"templateId":id}});
            },
            ...
            getAvailableActionItems: function() {
                ...
                if(data.availableActions.length == 0) {
                    $http.get("/proxy/v1/nflowmgr/sla/available-sla-template-actions").then(function (response) {
                        if (response.data) {
            ...
            validateTemplate:function(subject,templateString) {
                ...
                return $http({
                        url: "/proxy/v1/nflowmgr/sla/test-email-template",
                        method: "POST",
            ...
            sendTestEmail: function(address, subject, templateString) {
                ...
                return $http({
                    url: "/proxy/v1/nflowmgr/sla/send-test-email-template",
                    method: "POST",
                    data:angular.toJson(testTemplate),
                    headers: {
                        'Content-Type': 'application/json; charset=UTF-8'
            ...
            save:function(template){
                ...
                if(template != null) {
                    return $http({
                        url: "/proxy/v1/nflowmgr/sla/email-template",
                        method: "POST",
