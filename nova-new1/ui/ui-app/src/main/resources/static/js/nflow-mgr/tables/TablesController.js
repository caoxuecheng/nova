define(['angular',"nflow-mgr/tables/module-name"], function (angular,moduleName) {

    var controller = function($scope,$http,$q,RestUrlService, PaginationDataService,TableOptionsService, AddButtonService, NflowService,StateService,Utils){
        ...
        function getTables(){
            ...
            var successFn = function (response) {
              ...
              var nflowNames = response.nflowNames.data;
                var arr = [];
                if(_tables) {
                ...
                var filteredTables = self.selectedTables = _.filter(arr,function(t) {
                    return _.indexOf(nflowNames,t.lowerFullName) >0 || (!(Utils.endsWith(t.tableName, "_valid") || Utils.endsWith(t.tableName,"_invalid") || Utils.endsWith(t.tableName, "_profile") || Utils.endsWith(t.tableName,"_nflow")))});
                self.loading = false;
            ...
            var promises = {"hive":$http.get(RestUrlService.HIVE_SERVICE_URL+"/tables"),
                "nflowNames": NflowService.getNflowNames() };
        ...
        self.onClickTable = function(table){
            StateService.NflowManager().Table().navigateToTable(table.schema,table.tableName);
        }
        getTables();

    };

    angular.module(moduleName).controller('TablesController',["$scope","$http","$q","RestUrlService","PaginationDataService","TableOptionsService","AddButtonService","NflowService","StateService","Utils",controller]);
