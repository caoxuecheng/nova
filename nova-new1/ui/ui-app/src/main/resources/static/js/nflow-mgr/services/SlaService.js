define(['angular','nflow-mgr/module-name'], function (angular,moduleName) {
    angular.module(moduleName).factory('SlaService', ["$http","$q","$mdToast","$mdDialog","RestUrlService",function ($http, $q, $mdToast, $mdDialog, RestUrlService) {
        ...
        var data = {
            ...
            saveNflowSla: function (nflowId, serviceLevelAgreement) {
                var successFn = function (response) {
                    return response.data;
                }
                var errorFn = function (err) {
                ...
                var promise = $http({
                    url: RestUrlService.SAVE_NFLOW_SLA_URL(nflowId),
                    method: "POST",
            ...
            getNflowSlas: function (nflowId) {
                ...
                var promise = $http.get(RestUrlService.GET_NFLOW_SLA_URL(nflowId));
                promise.then(successFn, errorFn);
                return promise;
