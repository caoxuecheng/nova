define(['angular', 'nflow-mgr/nflows/define-nflow/module-name'], function (angular, moduleName) {
    ...
    var directive = function () {
        return {
            ...
            controllerAs: 'vm',
            require: ['onescorpinDefineNflowDataProcessing', '^onescorpinStepper'],
            scope: {},
            templateUrl: 'js/nflow-mgr/nflows/define-nflow/nflow-details/define-nflow-data-processing.html',
            controller: "DefineNflowDataProcessingController",
    ...
    var controller = function ($scope, $http, $mdDialog, $mdExpansionPanel, RestUrlService, NflowService, BroadcastService, StepperService, Utils, DomainTypesService, NflowTagService) {
        var self = this;
        ...
        NflowService.updateEnabledMergeStrategy(self.model, self.mergeStrategies);
        ...
        function onActiveStep(event, index) {
            if (index == parseInt(self.stepIndex)) {
                ...
                _.each(self.model.table.tableSchema.fields, function (columnDef, idx) {
                    columnDef.dataTypeDisplay = NflowService.getDataTypeDisplay(columnDef);
        ...
        this.allCompressionOptions = NflowService.compressionOptions;
        ...
        function validateMergeStrategies() {
            var validPK = NflowService.enableDisablePkMergeStrategy(self.model, self.mergeStrategies);

            self.dataProcessingForm['targetMergeStrategy'].$setValidity('invalidPKOption', validPK);

            validRollingSync = NflowService.enableDisableRollingSyncMergeStrategy(self.model, self.mergeStrategies);
        ...
        self.onDomainTypeChange = function (policy) {
            ...
            if ((domainType.field.derivedDataType !== null && (domainType.field.derivedDataType !== policy.field.derivedDataType || domainType.field.precisionScale !== policy.field.precisionScale))
                ...
                $mdDialog.show({
                    ...
                    .then(function () {
                        NflowService.setDomainTypeForField(policy.field, policy, domainType);
                    }, function () {
    ...
    angular.module(moduleName)
        .controller('DefineNflowDataProcessingController', ["$scope", "$http", "$mdDialog", "$mdExpansionPanel", "RestUrlService", "NflowService", "BroadcastService", "StepperService", "Utils",
                                                           "DomainTypesService", "NflowTagService", controller])
        .directive('onescorpinDefineNflowDataProcessing', directive);
});
