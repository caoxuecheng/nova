define(['angular',"nflow-mgr/templates/module-name"], function (angular,moduleName) {
    ...
    /**
     * Displays the page for registering a new Nflow Manager template.
     *
     * @constructor
    ...
    function RegisterNewTemplateController($scope, AccessControlService, StateService, RegisterTemplateService) {
        ...
        /**
         * Creates a new Nflow Manager template from a NiFi template.
         */
        self.createFromNifi = function() {
            RegisterTemplateService.resetModel();
            StateService.NflowManager().Template().navigateToRegisterNiFiTemplate();
        };

        /**
         * Imports a Nflow Manager template from a file.
         */
        self.importFromFile = function() {
            RegisterTemplateService.resetModel();
            StateService.NflowManager().Template().navigateToImportTemplate();
        };
        ...
        AccessControlService.getUserAllowedActions()
                .then(function(actionSet) {
                    if (AccessControlService.hasAction(AccessControlService.TEMPLATES_IMPORT, actionSet.actions)) {
                        self.registrationMethods.push({
                            name: "Import from a file", description: "Import from a Nova archive or NiFi template file", icon: "file_upload",
