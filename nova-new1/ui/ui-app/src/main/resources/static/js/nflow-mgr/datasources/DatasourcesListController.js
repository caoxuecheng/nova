define(["angular", "nflow-mgr/datasources/module-name"], function (angular, moduleName) {
    ...
    var DatasourcesListController = function ($scope, AccessControlService, AddButtonService, DatasourcesService, PaginationDataService, StateService, TableOptionsService) {
        ...
        self.sortOptions = (function () {
            var fields = {"Name": "name", "Description": "description", "Related Nflows": "sourceForNflows.length", "Type": "type"};
        ...
        self.editDatasource = function (datasource) {
            StateService.NflowManager().Datasource().navigateToDatasourceDetails(datasource.id);
        };

        /**
         * Gets the number of related nflows for the specified data source.
         *
         * @param {Object} datasource the data source
         * @returns {number} the number of related nflows
         */
        self.getRelatedNflowsCount = function (datasource) {
            return angular.isArray(datasource.sourceForNflows) ? datasource.sourceForNflows.length : 0;
        };
            ...
            .then(function (actionSet) {
                if (AccessControlService.hasAction(AccessControlService.DATASOURCE_EDIT, actionSet.actions)) {
                    AddButtonService.registerAddButton("datasources", function () {
                        StateService.NflowManager().Datasource().navigateToDatasourceDetails(null);
                    });
                }
            });
