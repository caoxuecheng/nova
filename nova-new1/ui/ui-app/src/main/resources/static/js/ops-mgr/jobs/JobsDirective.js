define(['angular','ops-mgr/jobs/module-name'], function (angular,moduleName) {
    ...
    function JobsCardController($scope, $http,$mdDialog,$timeout,$mdMenu, $q, $mdToast, $mdPanel, OpsManagerJobService, TableOptionsService, PaginationDataService, StateService, IconService,
        ...
        if (this.hideNflowColumn == undefined) {
            this.hideNflowColumn = false;
        }
        ...
        function loadAdditionalMenuOptions() {
            var options = [];
            if (self.nflow) {
                //only show the abandon all on the nflows page that are unhealthy
        ...
        function selectedAdditionalMenuOption(item) {
            if (item.type == 'abandon_all') {
                ...
                $mdDialog.show({
                    ...
                    fullscreen: true,
                    locals: {
                        nflowName: self.nflowFilter
                    }
                });

                OpsManagerJobService.abandonAllJobs(self.nflowFilter, function () {
                    $mdDialog.hide();
                    BroadcastService.notify('ABANDONED_ALL_JOBS', {nflow: self.nflowFilter});
                    $mdToast.show(
                        $mdToast.simple()
                            .textContent('Abandoned all failed jobs for the nflow')
                            .hideDelay(3000)
                    );
                },function(err){
                    $mdDialog.hide();
                    $mdToast.show(
                        $mdToast.simple()
                            .textContent('Unable to abandonal all jobs for the nflow.  A unexpected error occurred.')
                            .hideDelay(3000)
                    );
        ...
        this.onOptionsMenuOpen = function (options) {
            if (self.nflow) {
                ...
                if (abandonOption != null && abandonOption != undefined) {
                    abandonOption.disabled = self.nflow.healthText != 'UNHEALTHY';
                }
            }
        }
        ...
        function loadJobs(force) {
            if (force || !self.refreshing) {
                ...
                if (self.nflowFilter) {
                    ...
                    params.filter += "jobInstance.nflow.name=="+self.nflowFilter;
                }
        ...
        this.jobDetails = function(event, job) {
            if(job.stream){
                StateService.OpsManager().Nflow().navigateToNflowStats(job.jobName);
            }else {
        ...
        this.filterHelpFields.push(newHelpItem("Filter on a nflow name", "nflow"));
     ...
     */
    var abandonAllDialogController = function ($scope, $mdDialog, $interval,nflowName) {
        var self = this;

        $scope.nflowName = nflowName;
        $scope.message = "Abandoning the failed jobs for "+nflowName;
        var counter = 0;
        var index = 0;
        var messages = [];
        messages.push("Still working. Abandoning the failed jobs for "+nflowName);
    ...
    angular.module(moduleName).controller('AbandonAllJobsDialogController', ["$scope","$mdDialog","$interval","nflowName",abandonAllDialogController]);
