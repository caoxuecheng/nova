...
define(["require", "exports", "@angular/core", "angular", "jquery", "underscore", "../services/query-engine"], function (require, exports, core_1, angular, $, _, query_engine_1) {
    ...
    var TransformDataComponent = (function () {
         ...
         */
        function TransformDataComponent($scope, $element, $q, $mdDialog, domainTypesService, RestUrlService, SideNavService, uiGridConstants, NflowService, BroadcastService, StepperService, WindowUnloadService) {
            this.$scope = $scope;
            this.$q = $q;
            ...
            this.NflowService = NflowService;
            ...
            var onLoad = function () {
                ...
                _this.$scope.$watch(function () { return _this.fieldPolicies; }, function () {
                    _this.fieldPolicies.forEach(function (policy) {
                        ...
                        if (policy.nflowFieldName !== policy.name) {
                            policy.nflowFieldName = policy.name;
                        }
                    });
                    _this.engine.setFieldPolicies(_this.fieldPolicies);
        ...
        TransformDataComponent.prototype.showProfileDialog = function () {
            ...
            this.$mdDialog.show({
                ...
                templateUrl: "js/nflow-mgr/visual-query/transform-data/profile-stats/profile-stats-dialog.html"
            });
        };
        ;
        ...
        ;
        /**
         * Update the nflow model when changing from this transform step to a different step
         */
        TransformDataComponent.prototype.onStepChange = function (event, changedSteps) {
            ...
            if (changedSteps.oldStep === thisIndex) {
                this.saveToNflowModel().then(function () {
        ...
        /**
         * Saves the current transformation to the nflow model.
         *
         * @returns {Promise} signals when the save is complete
         */
        TransformDataComponent.prototype.saveToNflowModel = function () {
            var _this = this;
            // Add unsaved filters
            this.addFilters();
            // Check if updates are necessary
            var nflowModel = this.NflowService.createNflowModel;
            var newScript = this.engine.getNflowScript();
            if (newScript === nflowModel.dataTransformation.dataTransformScript) {
                var result = this.$q.defer();
                result.reject(true);
                return result.promise;
            }
            // Populate Nflow Model from the Visual Query Model
            nflowModel.dataTransformation.dataTransformScript = newScript;
            nflowModel.dataTransformation.states = this.engine.save();
            nflowModel.table.existingTableName = "";
            nflowModel.table.method = "EXISTING_TABLE";
            nflowModel.table.sourceTableSchema.name = "";
            // Get list of fields
            var deferred = this.$q.defer();
            var fields = this.engine.getFields();
            if (fields !== null) {
                this.NflowService.setTableFields(fields, this.engine.getFieldPolicies());
                this.NflowService.syncTableFieldPolicyNames();
                this.engine.save();
                deferred.resolve(true);
            }
            else {
                this.query().then(function () {
                    _this.NflowService.setTableFields(_this.engine.getFields(), _this.engine.getFieldPolicies());
                    _this.NflowService.syncTableFieldPolicyNames();
                    _this.engine.save();
                    deferred.resolve(true);
                });
            }
            ...
            if (domainType) {
                ...
                this.fieldPolicies = this.engine.getColumns().map(function (column, index) {
                    ...
                    if (index < fieldPolicies_1.length) {
                        ...
                        fieldPolicy = _this.NflowService.newTableFieldPolicy(column.hiveColumnLabel);
                        fieldPolicy.fieldName = column.hiveColumnLabel;
                        fieldPolicy.nflowFieldName = column.hiveColumnLabel;
                    }
                    if (index === columnIndex) {
                        _this.NflowService.setDomainTypeForField({}, fieldPolicy, domainType);
                    }
                    return fieldPolicy;
                });
            }
    ...
    angular.module(moduleName).component("onescorpinVisualQueryTransform", {
        bindings: {
            engine: "=",
            fieldPolicies: "<?",
            heightOffset: "@",
        ...
        controller: ["$scope", "$element", "$q", "$mdDialog", "DomainTypesService", "RestUrlService", "SideNavService", "uiGridConstants", "NflowService", "BroadcastService", "StepperService",
            "WindowUnloadService", TransformDataComponent],
        controllerAs: "$td",
        require: {
            stepperController: "^onescorpinStepper"
        },
        templateUrl: "js/nflow-mgr/visual-query/transform-data/transform-data.component.html"
    });
});
