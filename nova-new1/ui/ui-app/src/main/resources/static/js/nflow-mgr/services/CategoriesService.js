define(['angular','nflow-mgr/module-name','constants/AccessConstants'], function (angular,moduleName) {
    angular.module(moduleName).factory('CategoriesService',["$q","$http","RestUrlService","AccessControlService","EntityAccessControlService", function ($q, $http, RestUrlService,AccessControlService,EntityAccessControlService) {
        ...
        function Builder() {
            var builder = {
                ...
                relatedNflows: function (related) {
                    builder._relatedNflows = related || 0;
                    return this;
                },
                icon: function (icon) {
                    builder._icon = icon;
                ...
                build: function () {
                    ...
                    category.relatedNflows = this._relatedNflows;
        ...
        function loadAll() {
            if (!loading) {
                ...
                loadingRequest = $http.get(RestUrlService.CATEGORIES_URL).then(function (response) {
                    ...
                    categories = response.data.map(function (category) {
                        ...
                        category.createNflow = false;
                        //if under entity access control we need to check if the user has the "CREATE_NFLOW" permission associated with the selected category.
                        //if the user doesnt have this permission they cannot create nflows under this category
                        if (AccessControlService.isEntityAccessControlled()) {
                            if (AccessControlService.hasEntityAccess(EntityAccessControlService.ENTITY_ACCESS.CATEGORY.CREATE_NFLOW, category, "category")) {
                                category.createNflow = true;
                            }
                        }
                        else {
                            category.createNflow = true;
            ...
            /**
             * A category for grouping similar nflows.
             *
             ...
             * @property {Array<Object>} relatedNflowSummaries the nflows within this category
             */
            ...
            var data = {
                ...
                update:function(savedCategory){
                    ...
                    if(angular.isDefined(savedCategory.id)) {
                        ...
                        savedCategory.createNflow = false;
                        //if under entity access control we need to check if the user has the "CREATE_NFLOW" permission associated with the selected category.
                        //if the user doesnt have this permission they cannot create nflows under this category
                        if (AccessControlService.isEntityAccessControlled()) {
                            if (AccessControlService.hasEntityAccess(EntityAccessControlService.ENTITY_ACCESS.CATEGORY.CREATE_NFLOW, savedCategory, "category")) {
                                savedCategory.createNflow = true;
                            }
                        }
                        else {
                            savedCategory.createNflow = true;
                ...
                save: function (category) {
                    ...
                    EntityAccessControlService.updateRoleMembershipsForSave(category.nflowRoleMemberships);
                ...
                populateRelatedNflows: function (category) {
                    var deferred = $q.defer();
                    this.getRelatedNflows(category).then(function(response) {
                        category.relatedNflowSummaries = response.data || [];
                ...
                getRelatedNflows: function (category) {
                    return $http.get(RestUrlService.CATEGORIES_URL + "/" + category.id + "/nflows");
                },
                ...
                newCategory: function () {
                    return {
                        ...
                        userFields: [],
                        userProperties: [],
                        relatedNflowSummaries: [],
                        securityGroups: [],
                        roleMemberships: [],
                        nflowRoleMemberships: [],
                        owner: null
                    };
                },
