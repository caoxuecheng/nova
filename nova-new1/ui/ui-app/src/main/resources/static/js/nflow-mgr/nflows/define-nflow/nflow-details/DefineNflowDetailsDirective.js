...
define(['angular','nflow-mgr/nflows/define-nflow/module-name'], function (angular,moduleName) {
    ...
    var directive = function() {
        return {
            ...
            require: ['onescorpinDefineNflowDetails', '^onescorpinStepper'],
            scope: {},
            controllerAs: 'vm',
            templateUrl: 'js/nflow-mgr/nflows/define-nflow/nflow-details/define-nflow-details.html',
            controller: "DefineNflowDetailsController",
    ...
    function DefineNflowDetailsController($scope, $http, RestUrlService, NflowService, RegisterTemplateService, NflowInputProcessorOptionsFactory, BroadcastService, StepperService,
                                         NflowDetailsProcessorRenderingHelper) {
        var self = this;
        ...
        var inputDatabaseType = ["com.onescorpin.nifi.GetTableData"]
        ...
        function findControllerServicesForProperty(property) {
            ...
            NflowService.findControllerServicesForProperty(property);
        }

        function matchInputProcessor(inputProcessor, inputProcessors){

            if (inputProcessor == null) {
                //input processor is null when nflow is being created
                return undefined;
            }
            ...
            if(angular.isDefined(self.model.cloned) && self.model.cloned == true) {
               ...
               var inputProcessors = self.model.inputProcessors;
                ...
                if (self.inputProcessors.length === 0 && !_.some(self.nonInputProcessors, function (processor) {
                    ...
                    var step = StepperService.getStep("DefineNflowStepper", parseInt(self.stepIndex));
                    if (step != null) {
                        step.skip = true;
                    }
                }
        ...
        function getRegisteredTemplate() {
            if (self.model.templateId != null && self.model.templateId != '') {
                ...
                var promise = $http.get(RestUrlService.GET_REGISTERED_TEMPLATE_URL(self.model.templateId),{params:{nflowEdit:true,allProperties:true}});
         ...
         */
        function validate() {
            self.isValid = self.loading == false && self.model.systemNflowName != '' && self.model.systemNflowName != null && self.model.templateId != null
        ...
        function updateInputProcessor(processorId) {
            ...
            var renderGetTableData = NflowDetailsProcessorRenderingHelper.updateGetTableDataRendering(processor, self.model.nonInputProcessors);
    ...
    angular.module(moduleName).controller('DefineNflowDetailsController', ["$scope","$http","RestUrlService","NflowService","RegisterTemplateService","NflowInputProcessorOptionsFactory","BroadcastService","StepperService","NflowDetailsProcessorRenderingHelper",DefineNflowDetailsController]);
    angular.module(moduleName).directive('onescorpinDefineNflowDetails', directive);
});
