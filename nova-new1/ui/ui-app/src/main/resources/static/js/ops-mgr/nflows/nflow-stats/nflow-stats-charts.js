define(['angular', 'ops-mgr/nflows/nflow-stats/module-name'], function (angular, moduleName) {
    ...
    var directive = function () {
        return {
            ...
            controllerAs: 'vm',
            templateUrl: 'js/ops-mgr/nflows/nflow-stats/nflow-stats-charts.html',
            controller: "NflowStatsChartsController",
    ...
    var controller = function ($scope, $element, $http, $interval, $timeout, $q, $mdToast,ProvenanceEventStatsService, NflowStatsService, Nvd3ChartService, OpsManagerNflowService, StateService) {
        var self = this;
         ...
         * @type {null}
         */
        this.lastNflowTimeChartRefresh = null;

        /** flag when the nflow time chart is loading **/
        this.showNflowTimeChartLoading = true;
         ...
         * @type {*}
         */
        self.summaryStatistics = NflowStatsService.summaryStatistics;

        var nflowChartLegendState = [];
        this.nflowChartData = [];
        this.nflowChartApi = {};
        this.nflowChartOptions = {};
        ...
        self.processorStatsFunctions = NflowStatsService.processorStatsFunctions();

        /**
         * The Nflow we are looking at
        ...
        self.nflowProcessorErrorsTable = {
         ...
         * @type {*}
         */
        self.nflowProcessorErrors = NflowStatsService.nflowProcessorErrors;
        ...
        /**
         * Navigate to the Nflow Manager Nflow Details
         * @param ev
         */
        self.gotoNflowDetails = function (ev) {
         ...
         */
        self.viewNewNflowProcessorErrors = function () {
            self.nflowProcessorErrors.viewAllData();
        };

        self.toggleNflowProcessorErrorsRefresh = function (autoRefresh) {
            if (autoRefresh) {
                self.nflowProcessorErrors.viewAllData();
                self.nflowProcessorErrors.autoRefreshMessage = 'enabled';
            }
            else {
                self.nflowProcessorErrors.autoRefreshMessage = 'disabled';
            }
        };
        ...
        self.onResetZoom = function() {
            if(self.isZoomed) {
                initiatePreventZoom();
                resetZoom();
                self.nflowChartOptions.chart.xDomain = [self.minTime,self.maxTime]
                self.nflowChartOptions.chart.yDomain = [self.minY,self.maxY]
                self.nflowChartApi.refresh();
        ...
        function setupChartOptions() {
            self.processorChartOptions = {
                chart: {
                    ...
                    yAxis: {
                        axisLabel: NflowStatsService.processorStatsFunctionMap[self.selectedProcessorStatisticFunction].axisLabel,
                    ...
                    legend: {
                        dispatch: {
                            stateChange: function (e) {
                                nflowChartLegendState = e.disabled;
                            }
                        }
                    },
        ...
        //    NflowStatsService.setTimeBoundaries(self.minTime, self.maxTime);
            buildProcessorChartData();
           ...
           if(self.zoomedMinTime != UNZOOMED_VALUE) {
                ...
                self.nflowChartOptions.chart.xDomain = [self.zoomedMinTime,self.zoomedMaxTime]
                var y = self.zoomMaxY > 0 ? self.zoomMaxY : self.maxY;
                self.nflowChartOptions.chart.yDomain = [0,self.maxY]
            }
            else  {
                self.nflowChartOptions.chart.xDomain = [self.minTime,self.maxTime];
                self.nflowChartOptions.chart.yDomain = [0,self.maxY]
            }
           self.nflowChartApi.update();
*/
     ...
     /*   $scope.$watch(
        ...
        */
        ...
        function refresh(){
            ...
            NflowStatsService.setTimeBoundaries(from, to);
            buildChartData(true);
        }
        ...
        function enableZoom(){
            ...
            self.nflowChartOptions.chart.zoom.enabled=true;
        ...
        function disableZoom(){
            resetZoom();
            self.zoomEnabled = false;
            self.nflowChartOptions.chart.zoom.enabled=false;
            self.forceChartRefresh = true;
        }


        function onProcessorChartFunctionChanged() {
            NflowStatsService.setSelectedChartFunction(self.selectedProcessorStatisticFunction);
            var chartData = NflowStatsService.changeProcessorChartDataFunction(self.selectedProcessorStatisticFunction);
            self.processorChartData[0].values = chartData.data;
            NflowStatsService.updateBarChartHeight(self.processorChartOptions, self.processorChartApi, chartData.data.length, self.selectedProcessorStatisticFunction);
        }

        function buildChartData(timeIntervalChange) {
            if (!NflowStatsService.isLoading()) {
                timeIntervalChange = angular.isUndefined(timeIntervalChange) ? false : timeIntervalChange;
                self.nflowTimeChartLoading = true;
                self.processChartLoading = true;
                buildProcessorChartData();
                buildNflowCharts();
                fetchNflowProcessorErrors(timeIntervalChange);
            }
            getNflowHealth();
        }
        ...
        function buildProcessorChartData() {
            ...
            $q.when(NflowStatsService.fetchProcessorStatistics(minTime,maxTime)).then(function (response) {
                self.summaryStatsData = NflowStatsService.summaryStatistics;
                updateSummaryKpis();
                self.processorChartData = NflowStatsService.buildProcessorDurationChartData();

                NflowStatsService.updateBarChartHeight(self.processorChartOptions, self.processorChartApi, self.processorChartData[0].values.length, self.selectedProcessorStatisticFunction);
        ...
        function buildNflowCharts() {

            self.nflowTimeChartLoading = true;
            $q.when(NflowStatsService.fetchNflowTimeSeriesData()).then(function (nflowTimeSeries) {

                self.minTime = nflowTimeSeries.time.startTime;
                self.maxTime = nflowTimeSeries.time.endTime;
                self.timeDiff = self.maxTime - self.minTime;
                ...
                if (nflowChartLegendState.length > 0) {
                    _.each(chartArr, function (item, i) {
                        item.disabled = nflowChartLegendState[i];
                    });
                }

                self.nflowChartData = Nvd3ChartService.toLineChartData(nflowTimeSeries.raw.stats, chartArr, 'minEventTime', null,self.minTime, self.maxTime);
                var max = Nvd3ChartService.determineMaxY(self.nflowChartData);
                if(self.isZoomed) {
                    max = self.zoomMaxY;
                }
                ...
                self.nflowChartOptions.chart.forceY = [0, max];
                if (self.nflowChartOptions.chart.yAxis.ticks != max) {
                    self.nflowChartOptions.chart.yDomain = [0, max];
                    var ticks = max;
                    if (ticks > 8) {
                        ticks = 8;
                    }
                    ...
                    self.nflowChartOptions.chart.yAxis.ticks = ticks;
                }
                ...
                if(self.isZoomed && (self.forceXDomain == true || self.zoomedMinTime != UNZOOMED_VALUE)) {
                    ...
                    self.nflowChartOptions.chart.xDomain = [self.zoomedMinTime,self.zoomedMaxTime]
                    var y = self.zoomMaxY > 0 ? self.zoomMaxY : self.maxY;
                    self.nflowChartOptions.chart.yDomain = [0,y]
                }
                ...
                initiatePreventZoom();
                if (self.nflowChartApi && self.nflowChartApi.refresh  && self.nflowChartApi.update) {
                ...
                self.nflowTimeChartLoading = false;
                self.lastNflowTimeChartRefresh = new Date().getTime();
            }, function () {
                self.nflowTimeChartLoading = false;
                self.lastNflowTimeChartRefresh = new Date().getTime();
            });
        ...
        /**
         * fetch and append the errors to the NflowStatsService.nflowProcessorErrors.data object
         * @param resetWindow optionally reset the nflow errors to start a new array of errors in the nflowProcessorErrors.data
         */
        function fetchNflowProcessorErrors(resetWindow) {
            self.nflowProcessorErrorsLoading = true;
            $q.when(NflowStatsService.fetchNflowProcessorErrors(resetWindow)).then(function (nflowProcessorErrors) {
                self.nflowProcessorErrorsLoading = false;
            }, function (err) {
                self.nflowProcessorErrorsLoading = false;
            });
        ...
        /**
         * Gets the Nflow Health
         */
        function getNflowHealth() {
            var successFn = function (response) {
                if (response.data) {
                    //transform the data for UI
                    if (response.data.nflowSummary) {
                        angular.extend(self.nflow, response.data.nflowSummary[0]);
                        self.nflow.nflowId = self.nflow.nflowHealth.nflowId;
                        if (self.nflow.running) {
                            self.nflow.displayStatus = 'RUNNING';
                        }
                        else {
                            self.nflow.displayStatus = 'STOPPED';
                        }
                    }
            ...
            $http.get(OpsManagerNflowService.SPECIFIC_NFLOW_HEALTH_URL(self.nflowName)).then(successFn, errorFn);
        }
         ...
         */
        function initCharts() {
            NflowStatsService.setNflowName(self.nflowName);
            setupChartOptions();
    ...
    angular.module(moduleName).controller('NflowStatsChartsController',
        ["$scope", "$element", "$http", "$interval", "$timeout", "$q","$mdToast", "ProvenanceEventStatsService", "NflowStatsService", "Nvd3ChartService", "OpsManagerNflowService", "StateService", controller]);

    angular.module(moduleName)
        .directive('novaNflowStatsCharts', directive);
