define(["require", "exports", "./spark-query-engine", "nflow-mgr/visual-query/module", "nflow-mgr/visual-query/module-require"], function (require, exports, spark_query_engine_1) {
    "use strict";
    ...
    describe("Class: SparkQueryEngine", function () {
        ...
        beforeEach(mocks.module("nova", "nova.nflowmgr", moduleName));
        // constructor
        ...
        // getNflowScript
        it("should get a nflow script", mocks.inject(function ($http, $mdDialog, $timeout, DatasourcesService, HiveService, RestUrlService, uiGridConstants, VisualQueryService) {
            // Create service
            var service = new spark_query_engine_1.SparkQueryEngine($http, $mdDialog, $timeout, DatasourcesService, HiveService, RestUrlService, uiGridConstants, VisualQueryService);
            service.setQuery("SELECT * FROM invalid", []);
            service.states_[0].columns = [
            ...
            expect(service.getNflowScript()).toBe("import org.apache.spark.sql._\n"
                + "var df = sqlContext.sql(\"SELECT * FROM invalid\")\n"
                + "df = df\n"
        ...
        it("should splice transformation array", mocks.inject(function ($http, $mdDialog, $timeout, DatasourcesService, HiveService, RestUrlService, uiGridConstants, VisualQueryService) {
            ...
            service.push(tern.parse("\"onescorpin\""), {});
            expect(service.getScript()).toBe("import org.apache.spark.sql._\n"
                + "var df = sqlContext.sql(\"SELECT * FROM invalid\")\n"
                + "df = df.limit(1000)\n"
                + "df = df.select(df(\"*\"), functions.lit(42))\n"
                + "df = df.select(df(\"*\"), functions.lit(\"onescorpin\"))\n"
                + "df\n");
