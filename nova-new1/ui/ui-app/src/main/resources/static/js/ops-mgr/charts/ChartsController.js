define(['angular','ops-mgr/charts/module-name','ops-mgr/charts/PivotTableUtil'], function (angular,moduleName) {
    ...
    var controller = function($scope, $element,$http,HttpService, OpsManagerJobService, OpsManagerNflowService ){
        var self = this;
        this.selectedNflowNames = ['All'];
        this.startDate = null;
        this.endDate = null;
        this.limitRows = 500;
        ...
        this.pivotConfig = {rendererName:"Job Details", aggregatorName: "Average",
            ...
            cols: ["Start Date"], rows: ["Nflow Name"],
        ...
        this.refreshPivotTable = function () {
            ...
            var filter = "";
            if(!_.contains(self.selectedNflowNames,'All') && self.selectedNflowNames.length >0){
                filter = addToFilter(filter, "nflowName==\"" + self.selectedNflowNames.join(',') + "\"");
                self.filtered = true;
            }
        ...
        function getNflowNames(){
            ...
            var successFn = function (response) {
                if (response.data) {
                    ...
                    self.nflowNames = _.unique(response.data);
                    self.nflowNames.unshift('All');
                }
            }
            var errorFn = function (err) {
            }
...
;        }
    ...
    angular.module(moduleName).controller('ChartsController',["$scope","$element","$http","HttpService","OpsManagerJobService","OpsManagerNflowService",controller]);
