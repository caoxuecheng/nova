define(['angular',"nflow-mgr/templates/module-name"], function (angular,moduleName) {
    ...
    var directive = function () {
        return {
            ...
            scope: {},
            controllerAs: 'vm',
            templateUrl: 'js/nflow-mgr/templates/template-stepper/register-template/register-template-step.html',
    ...
    function RegisterCompleteRegistrationController($scope, $http, $mdToast, $mdDialog, RestUrlService, StateService, RegisterTemplateService, EntityAccessControlService) {
        ...
        var initTemplateFlowData = function () {
            if (self.model.needsReusableTemplate) {
                RegisterTemplateService.fetchRegisteredReusableNflowInputPorts().then(function (response) {
                    ...
                    angular.forEach(self.model.reusableTemplateConnections, function (connection) {
                        ...
                        self.registerTemplateForm["port-" + connection.nflowOutputPortName].$setValidity("invalidConnection", true);
                        if (!angular.isDefined(self.connectionMap[connection.inputPortDisplayName])) {
                            connection.inputPortDisplayName = null;
                            //mark as invalid
                            self.registerTemplateForm["port-" + connection.nflowOutputPortName].$setValidity("invalidConnection", false);
                        }
                    });
        ...
        this.registerTemplate = function () {
            ...
            var successFn = function (response) {
                ...
                StateService.NflowManager().Template().navigateToRegisterTemplateComplete(message, self.model, null);
            }
        ...
        var showRegistrationInProgressDialog = function () {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'js/nflow-mgr/templates/template-stepper/register-template/register-template-inprogress-dialog.html',
                parent: angular.element(document.body),
                clickOutsideToClose: false,
                fullscreen: true,
                locals: {
        ...
        var showErrorDialog = function (message) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'js/nflow-mgr/templates/template-stepper/register-template/register-template-error-dialog.html',
                parent: angular.element(document.body),
    ...
    angular.module(moduleName).directive("onescorpinRegisterCompleteRegistration", directive);
});
...
function RegisterTemplateCompleteController(StateService) {
    ...
    this.gotIt = function () {
        StateService.NflowManager().Template().navigateToRegisteredTemplates();
    }
