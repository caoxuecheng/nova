define(['angular', 'nflow-mgr/nflows/define-nflow/module-name'], function (angular, moduleName) {
    ...
    var controller = function ($scope, $http, $mdDialog, $q, $transition$, AccessControlService, NflowService, NflowSecurityGroups, RestUrlService, StateService, UiComponentsService) {

        var self = this;

        /**
         * Indicates if nflows may be imported from an archive.
         * @type {boolean}
         */
        ...
        /**
         * The model for creating the nflow
         * @type {*}
         */
        ...
        /**
         * The total number of steps to deisplay and render for the nflow stepper
         * @type {null}
         */
         ...
         * @type {string}
         */
        self.model.stepperTemplateUrl= 'js/nflow-mgr/nflows/define-nflow/define-nflow-stepper.html'


        var requestedTemplate = $transition$.params().templateName || '';
        var requestedTemplateId = $transition$.params().templateId || '';
        var nflowDescriptor = $transition$.params().nflowDescriptor || '';
        self.model.nflowDescriptor = nflowDescriptor;
        ...
        function getRegisteredTemplates() {
            var successFn = function (response) {
                ...
                if (response.data) {
                    ...
                    if (self.cloning) {
                        self.model = NflowService.cloneNflow();
        ...
        /**
         * Navigate to the import nflow screen
         */
        this.gotoImportNflow = function () {
            StateService.NflowManager().Nflow().navigatetoImportNflow();
        };
        ...
        this.selectTemplate = function (template) {
            ...
            self.model.dataTransformationNflow = template.dataTransformation;
            ...
            //set the total pre-steps for this nflow to be 0. They will be taken from the templateTableOption
        ...
        self.onStepperInitialized = function (stepper) {
            var accessChecks = {entityAccess: AccessControlService.checkEntityAccessControlled(), securityGroups: NflowSecurityGroups.isEnabled()};
        ...
        function init() {
            ...
            var cloneNflowName = $transition$.params().bcExclude_cloneNflowName;
            ...
            if (self.cloning) {
                showCloningDialog(cloneNflowName);
            }
            // Fetch the allowed actions
            AccessControlService.getUserAllowedActions()
                .then(function (actionSet) {
                    self.allowImport = AccessControlService.hasAction(AccessControlService.NFLOWS_IMPORT, actionSet.actions);
                });
         ...
         * @param cloneNflowName
         */
        function showCloningDialog(cloneNflowName) {
            if (angular.isUndefined(cloneNflowName)) {
                cloneNflowName = "the nflow";
            }
            $mdDialog.show({
                templateUrl: 'js/nflow-mgr/nflows/define-nflow/clone-nflow-dialog.html',
                parent: angular.element(document.body),
                clickOutsideToClose: true,
                locals: {
                    nflowName: cloneNflowName
                },
                controller: CloningDialogController,
                fullscreen: true
            });
            function CloningDialogController($scope, $mdDialog, nflowName) {
    ...
    angular.module(moduleName).controller('DefineNflowController',
        ["$scope", "$http", "$mdDialog", "$q", "$transition$", "AccessControlService", "NflowService", "NflowSecurityGroups", "RestUrlService", "StateService",
