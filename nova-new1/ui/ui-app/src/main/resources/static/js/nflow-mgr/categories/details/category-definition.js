define(['angular', 'nflow-mgr/categories/module-name'], function (angular, moduleName) {
     ...
     * @param NflowSecurityGroups the nflow security groups service
     * @param NflowService the nflow service
     */
    function CategoryDefinitionController($scope, $mdDialog, $mdToast, $q, $timeout, $window, AccessControlService, EntityAccessControlService, CategoriesService, StateService, NflowSecurityGroups, NflowService) {
        var self = this;
         ...
         * @type {string[]}
         */
        var reservedCategoryNames = ['onescorpin']
        ...
        this.categorySecurityGroups = NflowSecurityGroups;
        ...
        /**
         * System Name states:
         * !new 0, !editable 0, !nflows 0 - not auto generated, can change
         * !new 0, !editable 0,  nflows 1 - not auto generated, cannot change
         * !new 0,  editable 1, !nflows 0 - not auto generated, editable
         * !new 0,  editable 1,  nflows 1 - invalid state (cannot be editable with nflows)
         *  new 1, !editable 0, !nflows 0 - auto generated, can change
         *  new 1, !editable 0,  nflows 1 - invalid state (new cannot be with nflows)
         *  new 1,  editable 1, !nflows 0 - not auto generated, editable
         *  new 1,  editable 1,  nflows 1 - invalid state (cannot be editable with nflows)
         */
        self.getSystemNameDescription = function() {
            ...
            // console.log("self.hasNflows() = " + self.hasNflows());

            if (!self.isNewCategory() && !self.isSystemNameEditable() && self.hasNoNflows()) {
                return "Can be customised";
            }
            if (!self.isNewCategory() && !self.isSystemNameEditable() && self.hasNflows()) {
                return "Cannot be customised because Category has Nflows";
            }
            if (!self.isNewCategory() && self.isSystemNameEditable() && self.hasNoNflows()) {
                return "System name is now editable";
            }
            if (!self.isNewCategory() && self.isSystemNameEditable() && self.hasNflows()) {
                return ""; //invalid state, cannot be both editable and have nflows!
            }
            if (self.isNewCategory() && !self.isSystemNameEditable() && self.hasNoNflows()) {
                return "Auto generated from Category Name, can be customised";
            }
            if (self.isNewCategory() && !self.isSystemNameEditable() && self.hasNflows()) {
                return ""; //invalid state, cannot be new and already have nflows
            }
            if (self.isNewCategory() && self.isSystemNameEditable() && self.hasNoNflows()) {
                return "System name is now editable";
            }
            if (self.isNewCategory() && self.isSystemNameEditable() && self.hasNflows()) {
                return ""; //invalid state, cannot be new with nflows
            }
            return "";
        };
        ...
        self.hasNflows = function() {
            return !self.hasNoNflows();
        };

        self.hasNoNflows = function() {
            return (!angular.isArray(self.model.relatedNflowSummaries) || self.model.relatedNflowSummaries.length === 0);
        };
        ...
        self.canDelete = function () {
            return self.allowDelete && (angular.isString(self.model.id) && self.hasNoNflows());
        };
        ...
        self.onCancel = function () {
            ...
            if (!angular.isString(self.model.id)) {
                StateService.NflowManager().Category().navigateToCategories();
            }
        };
        ...
        self.validateDisplayAndSystemName = function() {
            ...
            NflowService.getSystemName(newDisplayName)
        ...
        self.validateDisplayName = function() {
            ...
            NflowService.getSystemName(newName)
                .then(function (response) {
                    var systemName = response.data;
                    if (self.isNewCategory() && !self.isSystemNameEditable()) {
                        self.editModel.systemName = systemName;
     ...
     */
    function onescorpinCategoryDefinition() {
        return {
            controller: "CategoryDefinitionController",
            controllerAs: "vm",
            restrict: "E",
            scope: {},
            templateUrl: "js/nflow-mgr/categories/details/category-definition.html"
        };
    }

    angular.module(moduleName).controller('CategoryDefinitionController',
        ["$scope", "$mdDialog", "$mdToast", "$q", "$timeout", "$window", "AccessControlService", "EntityAccessControlService", "CategoriesService", "StateService", "NflowSecurityGroups", "NflowService",
         CategoryDefinitionController]);
    angular.module(moduleName).directive('onescorpinCategoryDefinition', onescorpinCategoryDefinition);
});
