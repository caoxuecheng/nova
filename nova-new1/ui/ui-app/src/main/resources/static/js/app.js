define([
    ...
    'nova-common',
    'nova-services',
    'nova-side-nav'], function (angular,lazyLoad,SVGMorpheus,d3,moment) {
    'use strict';
    ...
    if(!window.moment){
     ...
     var module = angular.module("nova", ['ui.router', 'ui.router.upgrade', 'oc.lazyLoad', 'ngMaterial','material.components.expansionPanels','md.data.table','ngMdIcons',
                                          'angularUtils.directives.dirPagination','nova.common','nova.services','nova.side-nav','ngFx','ngAnimate','ngSanitize','ngTextTruncate', 'ui.grid',
    ...
    module.config(['$mdAriaProvider','$mdThemingProvider','$mdIconProvider','$urlServiceProvider','ngMdIconServiceProvider','$qProvider', function($mdAriaProvider,$mdThemingProvider, $mdIconProvider, $urlService, ngMdIconServiceProvider,$qProvider){
       //disable the aria-label warnings in the console
        ...
        $mdThemingProvider.theme('nova')
            .primaryPalette('primaryBlue', {
                'hue-2':'900'
            })
            .accentPalette('accentOrange');

        $mdThemingProvider.setDefaultTheme('nova');
    ...
    module.run(['$ocLazyLoad',function($ocLazyLoad){
        $ocLazyLoad.load({name:'nova',files:['bower_components/angular-material-icons/angular-material-icons.css',
