define(['angular', 'nflow-mgr/module-name','constants/AccessConstants'], function (angular, moduleName, AccessConstants) {
        ...
        var data = {
            showAccessControlDialog: function (entity, entityType, entityTitle, onSave, onCancel) {
                ...
                return $mdDialog.show({
                    ...
                    templateUrl: 'js/nflow-mgr/shared/entity-access-control/entity-access-control-dialog.html',
    ...
    angular.module(moduleName).factory('EntityAccessControlService', ["$http", "$q", "AccessControlService", "RestUrlService", function ($http, $q, AccessControlService, RestUrlService) {
        ...
        function queryForRoleAssignments(entity, membersType) {
            if (entity && entity.id && entity.id != null) {
                var url = '';
                if (membersType === 'nflow') {
                    url = RestUrlService.NFLOW_ROLES_URL(entity.id);
                } else if (membersType === 'category') {
                    url = RestUrlService.CATEGORY_ROLES_URL(entity.id);
                } else if (membersType === 'category-nflow') {
                	url = RestUrlService.CATEGORY_NFLOW_ROLES_URL(entity.id);
        ...
        var data = angular.extend(svc, {
            entityRoleTypes: {CATEGORY: "category", CATEGORY_NFLOW: "category-nflow", NFLOW: "nflow", TEMPLATE: "template", DATASOURCE: "datasource"},
            ...
            saveRoleMemberships: function (entityType,entityId, roleMemberships, callbackFn) {
                ...
                var url = '';
                if (entityType === 'nflow') {
                    url = RestUrlService.NFLOW_ROLES_URL(entityId);
