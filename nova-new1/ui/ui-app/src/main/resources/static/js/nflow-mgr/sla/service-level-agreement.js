define(['angular',"nflow-mgr/sla/module-name"], function (angular,moduleName) {
    ...
    var directive = function() {
        return {
            ...
            templateUrl: function(tElement, tAttrs) {
                if (tAttrs) {
                    if (tAttrs.view === 'all') {
                        return 'js/nflow-mgr/sla/service-level-agreements.html'
                    }
                    if (tAttrs.view === 'nflow') {
                        return 'js/nflow-mgr/sla/nflow-service-level-agreements.html'
                    }
                }
            },
    ...
    var controller = function($scope, $mdDialog, $mdToast, $http, $rootScope, $q,StateService, NflowService, SlaService, PolicyInputFormService, PaginationDataService, TableOptionsService,
        ...
        /**
         * shows the (+) Add button when trying to assign an SLA that is not directly tied to a nflow initially
         * @type {boolean}
         */
        ...
        function showList(requery) {
            ...
            //Requery?
            if (self.nflow == null && self.allowCreate) {
                //display the add button if we are not in a given nflow
                AddButtonService.showAddButton();
            }

        }

        //   this.nflow = NflowService.editNflowModel;
        ...
        /**
         * The Current array of SLA objects found for this nflow
         ...
         * If there is a Nflow using this page it will only get the SLAs related for this nflow
        ...
        function loadSlas() {
            ...
            self.loading = true;
            /**
             * Load and copy the serviceLevelAgreements from the nflow if available
             * @type {Array|*}
             */
            if (self.nflow) {
                var arr = self.nflow.serviceLevelAgreements;
            ...
            if (self.nflow != null) {
                SlaService.getNflowSlas(self.nflow.nflowId).then(function(response) {
        ...
        SlaService.getPossibleSlaMetricOptions().then(function(response) {
            ...
            var currentNflowValue = null;
            if (self.nflow != null) {
                currentNflowValue = PolicyInputFormService.currentNflowValue(self.nflow);
            }
            self.options = PolicyInputFormService.groupPolicyOptions(response.data, currentNflowValue);
            if(self.allowCreate || self.allowEdit){
                PolicyInputFormService.stripNonEditableNflows(self.options);
            }
        ...
        SlaService.getPossibleSlaActionOptions().then(function(response) {
            ...
            self.slaActionOptions = PolicyInputFormService.groupPolicyOptions(response.data, currentNflowValue);
            if (self.slaActionOptions.length > 0) {
                ...
                if(self.allowCreate || self.allowEdit){
                    PolicyInputFormService.stripNonEditableNflows(self.slaActionOptions);
                }
        ...
        function deriveSlaName(){
            ...
            var nflowNamesString = null;
            var nflowNames = PolicyInputFormService.getNflowNames(self.editSla.rules);
            if(nflowNames.length >0) {
                nflowNamesString = nflowNames.join(",");
            }
            var ruleNames = PolicyInputFormService.getRuleNames(self.editSla.rules);
            var slaName = ruleNames.join(",");
            if(nflowNamesString != null) {
                slaName = nflowNamesString + " - " + slaName;
            }
            return slaName;
        }
        ...
        function deriveDescription(){
            ...
            if(nflowNamesString != null) {
                desc += " for "+nflowNamesString;
            }
            return desc;
        }

        self.onPropertyChange = function(property) {
            if(PolicyInputFormService.isNflowProperty(property)) {
    ...
    function saveSla(successFn, failureFn) {
        ...
        if (self.nflow != null) {
            SlaService.saveNflowSla(self.nflow.nflowId, self.editSla).then(function(response) {
                $mdDialog.hide();
                if (successFn) {
                    successFn(response);
                }
    ...
    function applyEditPermissionsToSLA(sla){
        ...
        $q.when(functionalAccess).then(function (response) {
            if(entityAccessControlled) {
                ...
                var allowNflowEdit = self.nflow != null ? AccessControlService.hasAction(AccessControlService.NFLOWS_EDIT, response.actions) : true;
                self.allowEdit =  allowNflowEdit && AccessControlService.hasAction(AccessControlService.SLA_EDIT, response.actions);
    ...
    function applyAccessPermissions() {
        ...
        if (self.editSla && !self.newSla) {
            ...
            //ensure the user has the EDIT_SLA and if editing for a nflow ensure the user has access to edit that nflow

            var entityAccessControlled = self.nflow != null && AccessControlService.isEntityAccessControlled();

            //Apply the entity access permissions
            var requests = {
                entityEditAccess: entityAccessControlled == true ? NflowService.hasEntityAccess(EntityAccessControlService.ENTITY_ACCESS.NFLOW.EDIT_NFLOW_DETAILS, self.nflow) : true,
            ...
            $q.all(requests).then(function (response) {
                ...
                var allowNflowEdit = self.nflow != null ? AccessControlService.hasAction(AccessControlService.NFLOWS_EDIT, response.functionalAccess.actions) : true;
                self.allowEdit = response.entityEditAccess && allowEditAccess && slaAccess && allowNflowEdit;
            });
...
angular.module(moduleName).controller('ServiceLevelAgreementController', ["$scope","$mdDialog","$mdToast","$http","$rootScope","$q","StateService","NflowService","SlaService","PolicyInputFormService","PaginationDataService","TableOptionsService","AddButtonService","AccessControlService","EntityAccessControlService",controller]);
angular.module(moduleName)
    .directive('onescorpinServiceLevelAgreement', directive);
