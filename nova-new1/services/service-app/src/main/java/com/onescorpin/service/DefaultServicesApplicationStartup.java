package com.onescorpin.service;
...
import com.onescorpin.app.ApplicationStartupListenerType;
...
public class DefaultServicesApplicationStartup implements ServicesApplicationStartup, ApplicationListener<ContextRefreshedEvent> {
    ...
    private void determineApplicationType(final ContextRefreshedEvent event) {
        applicationType = ApplicationType.NOVA;
        String[] activeProfiles = event.getApplicationContext().getEnvironment().getActiveProfiles();

        if (activeProfiles != null && Arrays.asList(activeProfiles).contains(NovaUpgrader.NOVA_UPGRADE)) {
    ...
    private class StartupTask implements Runnable {
        ...
        public void run() {
            ...
            if (a != null) {
                if (a.listenerType() == ApplicationStartupListenerType.ListenerType.UPGRADE_AND_NOVA ||
                    (applicationType == ApplicationType.UPGRADE && a.listenerType() == ApplicationStartupListenerType.ListenerType.UPGRADE_AND_NOVA ||
                     applicationType == ApplicationType.NOVA && a.listenerType() == ApplicationStartupListenerType.ListenerType.NOVA_ONLY)) {
