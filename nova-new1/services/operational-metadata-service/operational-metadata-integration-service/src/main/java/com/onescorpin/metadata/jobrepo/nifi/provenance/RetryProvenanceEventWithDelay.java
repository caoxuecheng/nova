...
/**
 * Retry processing events if not registered in Nova
 */
public class RetryProvenanceEventWithDelay {
    ...
    @Value("${nova.provenance.retry.unregistered.events.enabled:true}")
    private boolean enabled = true;

    @Value("${nova.provenance.retry.unregistered.events.waitTimeSec:5}")
    private Integer waitTimeSec = 5;

    @Value("${nova.provenance.retry.unregistered.events.maxRetries:3}")
    ...
    BlockingQueue<RetryAggregatedNflowProcessorStatisticsHolder> statsQueue = new LinkedBlockingQueue<>();
    ...
    public void delay(AggregatedNflowProcessorStatisticsHolder statsHolder, List<AggregatedNflowProcessorStatistics> unregisteredEvents) {
        if (enabled) {
            RetryAggregatedNflowProcessorStatisticsHolder holder = null;
            if (statsHolder instanceof RetryAggregatedNflowProcessorStatisticsHolder) {
                holder = (RetryAggregatedNflowProcessorStatisticsHolder) statsHolder;
                holder.setLastRetryTime(DateTime.now());
                holder.incrementRetryAttempt();
            } else {
                holder = new RetryAggregatedNflowProcessorStatisticsHolder(maxRetries);
            }
            holder.setNflowStatistics(unregisteredEvents);
    ...
    private void processStatsQueue() {
        try {
            List<AggregatedNflowProcessorStatisticsHolder> stats = new ArrayList<>();
