...
import com.onescorpin.nflowmgr.rest.model.IconColor;
import com.onescorpin.nflowmgr.rest.support.SystemNamingService;
import com.onescorpin.nflowmgr.service.UIService;
import com.onescorpin.json.ObjectMapperSerializer;
import com.onescorpin.nflowmgr.security.EncryptionAccessControl;
...
@Api(tags = "Nflow Manager - Utilities", produces = "application/json")
@Path("/v1/nflowmgr/util")
@Component
public class UtilityRestController {
    ...
    public Response partitionFunctions() {
        final Stream<String> novaFunctions = Stream.of("val", "to_date", "year", "month", "day", "hour", "minute");
        final Stream<String> userFunctions = Arrays.stream(env.getProperty("nova.metadata.udfs", "").split(",")).map(String::trim).filter(StringUtils::isNotEmpty);
        return Response.ok(Stream.concat(novaFunctions, userFunctions).collect(Collectors.toSet())).build();
    }
