...
import com.onescorpin.metadata.api.datasource.DatasourceDetails;
...
import com.onescorpin.metadata.rest.model.data.DerivedDatasource;
import com.onescorpin.metadata.rest.model.data.JdbcDatasource;
import com.onescorpin.metadata.rest.model.data.UserDatasource;
import com.onescorpin.metadata.rest.model.nflow.Nflow;
import com.onescorpin.nifi.rest.client.NiFiControllerServicesRestClient;
...
public class DatasourceModelTransform {
     ...
     */
    public Datasource toDatasource(@Nonnull final com.onescorpin.metadata.api.datasource.Datasource domain, @Nonnull final Level level) {
        if (domain instanceof com.onescorpin.metadata.api.datasource.DerivedDatasource) {
            final DerivedDatasource ds = new DerivedDatasource();
            updateDatasource(ds, (com.onescorpin.metadata.api.datasource.DerivedDatasource) domain, level);
            return ds;
        } else if (domain instanceof com.onescorpin.metadata.api.datasource.UserDatasource) {
            return toDatasource((com.onescorpin.metadata.api.datasource.UserDatasource) domain, level);
        } else {
     ...
     * @throws IllegalArgumentException if the domain object cannot be converted
     */
    public UserDatasource toDatasource(@Nonnull final com.onescorpin.metadata.api.datasource.UserDatasource domain, @Nonnull final Level level) {
     ...
     */
    public com.onescorpin.metadata.api.datasource.Datasource toDomain(@Nonnull final Datasource ds) {
        if (ds instanceof UserDatasource) {
            final com.onescorpin.metadata.api.datasource.UserDatasource domain;
            final boolean isNew = (ds.getId() == null);

            if (isNew) {
                domain = datasourceProvider.ensureDatasource(ds.getName(), ds.getDescription(), com.onescorpin.metadata.api.datasource.UserDatasource.class);
                ds.setId(domain.getId().toString());
            } else {
                final com.onescorpin.metadata.api.datasource.Datasource.ID id = datasourceProvider.resolve(ds.getId());
                domain = (com.onescorpin.metadata.api.datasource.UserDatasource) datasourceProvider.getDatasource(id);
            }
     ...
     * @param level  the level of detail
     */
    private void updateDatasource(@Nonnull final Datasource ds, @Nonnull final com.onescorpin.metadata.api.datasource.Datasource domain, @Nonnull final Level level) {
        ds.setId(domain.getId().toString());
        ...
        if (level.compareTo(Level.CONNECTIONS) <= 0) {
            for (com.onescorpin.metadata.api.nflow.NflowSource domainSrc : domain.getNflowSources()) {
                ...
                nflow.setDisplayName(domainSrc.getNflow().getDisplayName());
                nflow.setId(domainSrc.getNflow().getId().toString());
                nflow.setState(Nflow.State.valueOf(domainSrc.getNflow().getState().name()));
                nflow.setSystemName(domainSrc.getNflow().getName());
                nflow.setModifiedTime(domainSrc.getNflow().getModifiedTime());

                ds.getSourceForNflows().add(nflow);
            }
            for (com.onescorpin.metadata.api.nflow.NflowDestination domainDest : domain.getNflowDestinations()) {
                Nflow nflow = new Nflow();
                nflow.setDisplayName(domainDest.getNflow().getDisplayName());
                nflow.setId(domainDest.getNflow().getId().toString());
                nflow.setState(Nflow.State.valueOf(domainDest.getNflow().getState().name()));
                nflow.setSystemName(domainDest.getNflow().getName());
                nflow.setModifiedTime(domainDest.getNflow().getModifiedTime());

                ds.getDestinationForNflows().add(nflow);
            }
        }
    }
     ...
     * @param level  the level of detail
     */
    public void updateDatasource(@Nonnull final DerivedDatasource ds, @Nonnull final com.onescorpin.metadata.api.datasource.DerivedDatasource domain, @Nonnull final Level level) {
     ...
     * @param level  the level of detail
     */
    private void updateDatasource(@Nonnull final JdbcDatasource ds, @Nonnull final com.onescorpin.metadata.api.datasource.UserDatasource domain, @Nonnull final Level level) {
     ...
     * @param level  the level of detail
     */
    private void updateDatasource(@Nonnull final UserDatasource ds, @Nonnull final com.onescorpin.metadata.api.datasource.UserDatasource domain, @Nonnull final Level level) {
        updateDatasource((Datasource) ds, domain, level);
     ...
     * @param ds     the REST object
     */
    private void updateDomain(@Nonnull final com.onescorpin.metadata.api.datasource.UserDatasource domain, @Nonnull final JdbcDatasource ds) {
        updateDomain(domain, (UserDatasource) ds);
        domain.getDetails()
            .map(JdbcDatasourceDetails.class::cast)
            .ifPresent(details -> {
     ...
     * @param ds     the REST object
     */
    private void updateDomain(@Nonnull final com.onescorpin.metadata.api.datasource.UserDatasource domain, @Nonnull final UserDatasource ds) {
        // Update properties
