...
/**
 * in memory implementation of the NflowTemplateService for use in testing
 */
public class InMemoryNflowManagerTemplateService implements NflowManagerTemplateService {


    @Inject
    private NflowManagerNflowService nflowProvider;
    ...
    protected RegisteredTemplate saveRegisteredTemplate(RegisteredTemplate registeredTemplate) {
        ...
        if (template != null && !template.getId().equalsIgnoreCase(registeredTemplate.getId())) {
            ...
            //update those nflows that were pointing to this old one, to this new one

            List<NflowMetadata> nflowsToUpdate = nflowProvider.getNflowsWithTemplate(registeredTemplate.getId());
            if (nflowsToUpdate != null && !nflowsToUpdate.isEmpty()) {
                for (NflowMetadata nflowMetadata : nflowsToUpdate) {
                    nflowMetadata.setTemplateId(template.getId());
                }
                //save the nflows
                FileObjectPersistence.getInstance().writeNflowsToFile(nflowProvider.getNflows());
            }
        }
