...
import com.onescorpin.nflowmgr.nifi.CleanupStaleNflowRevisions;
...
import com.onescorpin.nifi.rest.model.NiFiClusterSummary;
import com.onescorpin.nifi.rest.model.NiFiPropertyDescriptorTransform;
import com.onescorpin.nifi.rest.model.flow.NifiFlowDeserializer;
...
@Api(tags = "Nflow Manager - NiFi", produces = "application/json")
@Path(NifiIntegrationRestController.BASE)
@Component
@SwaggerDefinition(tags = @Tag(name = "Nflow Manager - NiFi", description = "integration with NiFi"))
public class NifiIntegrationRestController {
     ...
     */
    private static final ResourceBundle STRINGS = ResourceBundle.getBundle("com.onescorpin.nflowmgr.rest.controller.NiFiIntegrationMessages");
    public static final String BASE = "/v1/nflowmgr/nifi";
    ...
    public Response cleanupVersionedProcessGroups(@PathParam("processGroupId") String processGroupId) {
        ...
        RestResponseStatus status;
        CleanupStaleNflowRevisions cleanupStaleNflowRevisions = new CleanupStaleNflowRevisions(legacyNifiRestClient, processGroupId, propertyDescriptorTransform);
        cleanupStaleNflowRevisions.cleanup();
        String msg = "Cleaned up " + cleanupStaleNflowRevisions.getDeletedProcessGroups().size() + " Process Groups";
    ...
    public Response getFlow(@PathParam("processGroupId") String processGroupId) {
        ...
        NifiFlowProcessGroup flow = legacyNifiRestClient.getNflowFlow(processGroupId);
        NifiFlowDeserializer.prepareForSerialization(flow);
        return Response.ok(flow).build();
    }

    @GET
    @Path("/flow/nflow/{categoryAndNflowName}")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Gets the flow of the specified nflow.")
    @ApiResponses({
                      @ApiResponse(code = 200, message = "Returns the flow.", response = NifiFlowProcessGroup.class),
                      @ApiResponse(code = 500, message = "The process group is unavailable.", response = RestResponseStatus.class)
                  })
    public Response getFlowForCategoryAndNflow(@PathParam("categoryAndNflowName") String categoryAndNflowName) {
        accessController.checkPermission(AccessController.SERVICES, NflowServicesAccessControl.ADMIN_NFLOWS);

        NifiFlowProcessGroup flow = legacyNifiRestClient.getNflowFlowForCategoryAndNflow(categoryAndNflowName);
        NifiFlowDeserializer.prepareForSerialization(flow);
        return Response.ok(flow).build();
    }
    ...
    public Response getFlows() {
        ...
        List<NifiFlowProcessGroup> nflowFlows = legacyNifiRestClient.getNflowFlows();
        if (nflowFlows != null) {
            log.info("********************** getAllFlows  ({})", nflowFlows.size());
            nflowFlows.stream().forEach(group -> NifiFlowDeserializer.prepareForSerialization(group));
        }
        return Response.ok(nflowFlows).build();
    }
    ...
    public Response getReusableNflowInputPorts() {
        Set<PortDTO> ports = nflowManagerTemplateService.getReusableNflowInputPorts();
    ...
    @ApiOperation(value = "Executes a SELECT query.",
                  notes = "Connects to the database specified by the controller service using the password defined in Nova's application.properties file.")
    @ApiResponses({
