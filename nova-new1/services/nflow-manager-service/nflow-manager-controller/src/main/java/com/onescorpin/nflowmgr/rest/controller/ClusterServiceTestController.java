...
import com.onescorpin.cluster.ClusterNodeSummary;
import com.onescorpin.cluster.ClusterService;
import com.onescorpin.cluster.ClusterServiceTester;
import com.onescorpin.cluster.MessageDeliveryStatus;
import com.onescorpin.cluster.SimpleClusterMessageTest;
...
@Api(tags = "Nova Cluster - Administration", produces = "application/json")
@Path("/v1/admin/cluster")
@SwaggerDefinition(tags = @Tag(name = "Nova Cluster - Administration", description = "administrator cluster"))
public class ClusterServiceTestController {
    ...
    @Path("/is-clustered")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Check to see if Nova is clustered")
    @ApiResponses({
    ...
    @ApiResponses({
                      @ApiResponse(code = 200, message = "Returns the list of Nova cluster members", response = String.class, responseContainer = "List")
                  })
    ...
    @POST
    @Path("/simple")
    @ApiOperation("Send a Simple text message to other nodes in the Nova cluster")
    @ApiResponses({
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Gets messages awaiting acknowledgement.  This will only return data if the property: nova.cluster.acknowledge=true in the application.properties of nova-services ")
    @ApiResponses({
