...
import com.onescorpin.nflowmgr.rest.beanvalidation.NewNflowCategory;
...
/**
 * REST API for managing categories within the Nflow Manager.
 */
@Api(tags = "Nflow Manager - Categories", produces = "application/json")
@Path(NflowCategoryRestController.BASE)
@Component
@SwaggerDefinition(tags = @Tag(name = "Nflow Manager - Categories", description = "manages categories"))
public class NflowCategoryRestController {
    
    private static final Logger log = LoggerFactory.getLogger(NflowCategoryRestController.class);

    public static final String BASE = "/v1/nflowmgr/categories";
    ...
    @ApiResponses({
                      @ApiResponse(code = 200, message = "Returns the category details.", response = NflowCategory.class),
    ...
    public Response getCategoryByName(@PathParam("categorySystemName") String categorySystemName) {
        NflowCategory category = getMetadataService().getCategoryBySystemName(categorySystemName);
    ...
    public Response getCategoryById(@UUID @PathParam("categoryId") String categoryId) {
        NflowCategory category = getMetadataService().getCategoryById(categoryId);
    ...
    @ApiResponses(
        @ApiResponse(code = 200, message = "Returns the list of categories.", response = NflowCategory.class, responseContainer = "List")
    )
    public Response getCategories(@QueryParam("includeNflowDetails") @DefaultValue("false") boolean includeNflowDetails) {
        Collection<NflowCategory> categories = getMetadataService().getCategories(includeNflowDetails);
    ...
    @ApiResponses({
                      @ApiResponse(code = 200, message = "The category was saved successfully.", response = NflowCategory.class),
    ...
    public Response saveCategory(@NewNflowCategory NflowCategory nflowCategory) {
        getMetadataService().saveCategory(nflowCategory);
        //requery it to get the allowed actions
        NflowCategory savedCategory = getMetadataService().getCategoryBySystemName(nflowCategory.getSystemName());
    ...
    @GET
    @Path("/{categoryId}/nflows")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Gets the nflows for the specified category.")
    @ApiResponses({
                      @ApiResponse(code = 200, message = "The list of related nflows."),
                      @ApiResponse(code = 400, message = "The categoryId is invalid.", response = RestResponseStatus.class)
                  })
    public Response getCategory(@UUID @PathParam("categoryId") String categoryId) {
        List<NflowSummary> summaryList = getMetadataService().getNflowSummaryForCategory(categoryId);
    ...
    /**
     * Returns the user fields for nflows within the specified category.
     *
    ...
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Returns the user fields for nflows within the specified category.")
    @ApiResponses({
                      @ApiResponse(code = 200, message = "Returns the user fields.", response = UserProperty.class, responseContainer = "List"),
                      @ApiResponse(code = 400, message = "The categoryId is invalid.", response = RestResponseStatus.class)
                  })
    @Nonnull
    public Response getNflowUserFields(@Nonnull @PathParam("categoryId") @UUID final String categoryId) {
        final Set<UserProperty> userFields = getMetadataService().getNflowUserFields(categoryId).orElseThrow(NotFoundException::new);
        return Response.ok(userFields).build();
    }
    ...
    public Response getNflowRoleMemberships(@PathParam("categoryId") String categoryIdStr,
                                           @QueryParam("verbose") @DefaultValue("false") boolean verbose) {

        return this.securityService.getCategoryNflowRoleMemberships(categoryIdStr)
                        .map(m -> Response.ok(m).build())
                        .orElseThrow(() -> new WebApplicationException("A category with the given ID does not exist: " + categoryIdStr, Status.NOT_FOUND));
    }
    ...
    public Response postNflowRoleMembershipChange(@PathParam("categoryId") String categoryIdStr,
                                                 RoleMembershipChange changes) {
        return this.securityService.changeCategoryNflowRoleMemberships(categoryIdStr, changes)
                        .map(m -> Response.ok(m).build())
                        .orElseThrow(() -> new WebApplicationException("Either a category with the ID \"" + categoryIdStr
                                                                       + "\" does not exist or it does not have a role the named \"" 
                                                                       + changes.getRoleName() + "\"", Status.NOT_FOUND));
