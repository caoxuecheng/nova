...
import com.onescorpin.nflowmgr.service.template.TemplateModelTransform.TEMPLATE_TRANSFORMATION_TYPE;
...
import com.onescorpin.nifi.rest.support.NifiTemplateUtil;
...
public class RegisteredTemplateService {
    ...
    public boolean checkTemplatePermission(final String id, final Action action, final Action... more) {
        if (accessController.isEntityAccessControlled()) {
            return metadataAccess.read(() -> {
                final NflowManagerTemplate.ID domainId = templateProvider.resolveId(id);
                final NflowManagerTemplate domainTemplate = templateProvider.findById(domainId);
    ...
    public RegisteredTemplate findRegisteredTemplate(RegisteredTemplateRequest registeredTemplateRequest) {
        ...
        //if we are looking for a given template as a request from a Nflow, we need to query and access it via a service account.
        //otherwise we will access it as the user
        Principal[] principals = null;
        if (registeredTemplateRequest.isNflowEdit()) {
        ...
        //if requested as a template or nflow edit, it will include the encrypted sensitive property values
        TEMPLATE_TRANSFORMATION_TYPE transformationType = TEMPLATE_TRANSFORMATION_TYPE.WITH_NFLOW_NAMES;
        if (registeredTemplateRequest.isTemplateEdit() || registeredTemplateRequest.isNflowEdit() || registeredTemplateRequest.isIncludeSensitiveProperties()) {
        ...
        if (registeredTemplate != null) {
            ...
            if (NifiPropertyUtil.containsPropertiesForProcessorMatchingType(registeredTemplate.getProperties(), NifiNflowConstants.TRIGGER_NFLOW_PROCESSOR_CLASS)) {
    ...
    /**
     * Find a template by the Nova Id
     *
     * @param templateId The Nova {@link RegisteredTemplate#id}
    ...
    private RegisteredTemplate findRegisteredTemplateById(final String templateId, TEMPLATE_TRANSFORMATION_TYPE transformationType, Principal... principals) {
        if (StringUtils.isBlank(templateId)) {
            ...
            return metadataAccess.read(() -> {
                ...
                NflowManagerTemplate domainTemplate = templateProvider.findById(domainId);
                if (domainTemplate != null) {
                    //transform it
    ...
    private RegisteredTemplate findRegisteredTemplateByNiFiIdentifier(final String nifiTemplateId, TEMPLATE_TRANSFORMATION_TYPE transformationType, Principal... principals) {
        ...
        return metadataAccess.read(() -> {
            ...
            RegisteredTemplate registeredTemplate = null;
            NflowManagerTemplate template = templateProvider.findByNifiTemplateId(nifiTemplateId);
            if (template != null) {
    ...
    public RegisteredTemplate findRegisteredTemplateByName(final String templateName, TEMPLATE_TRANSFORMATION_TYPE transformationType, Principal... principals) {
        return metadataAccess.read(() -> {
            ...
            RegisteredTemplate registeredTemplate = null;
            NflowManagerTemplate template = templateProvider.findByName(templateName);
            if (template != null) {
                registeredTemplate = templateModelTransform.getTransformationFunction(transformationType).apply(template);
            }
            return registeredTemplate;
    ...
    /**
     * Return a registered template object that is populated for use with updating in Nova
     *
     * @param registeredTemplateRequest the request to get a registered template
     * @return a RegisteredTemplate object mapping either one already defined in Nova, or a new template that maps to one in NiFi
     */
    public RegisteredTemplate getRegisteredTemplateForUpdate(RegisteredTemplateRequest registeredTemplateRequest) {
        ...
        //editing a nflow will run as a service account
        serviceLevelRequest.setNflowEdit(true);
            ...
            if (registeredTemplate == null) {
                ...
                for (final PortDTO port : outputPorts) {
                    ...
                    if (reusableTemplateConnectionInfo == null) {
                        ...
                        reusableTemplateConnectionInfo.setNflowOutputPortName(port.getName());
                    }
    ...
    public List<RegisteredTemplate> getRegisteredTemplates() {
        return metadataAccess.read(() -> {
            ...
            List<NflowManagerTemplate> templates = templateProvider.findAll();
            if (templates != null) {
                templates.stream().filter(t -> t.getNifiTemplateId() == null).forEach(t -> {
                    ensureNifiTemplateId(t);
                });

                registeredTemplates = templateModelTransform.domainToRegisteredTemplateWithNflowNames(templates);
     ...
     * This is to be used for new templates that are going to be registered with Nova.
    ...
    public NflowMetadata mergeTemplatePropertiesWithNflow(NflowMetadata nflowMetadata) {
        //gets the nflow data and then gets the latest template associated with that nflow and merges the properties into the nflow

        RegisteredTemplate
            registeredTemplate =
            findRegisteredTemplate(
                new RegisteredTemplateRequest.Builder().templateId(nflowMetadata.getTemplateId()).isNflowEdit(true).nifiTemplateId(nflowMetadata.getTemplateId()).includeAllProperties(true).build());
        if (registeredTemplate != null) {
            nflowMetadata.setTemplateId(registeredTemplate.getId());
            ...
            NifiPropertyUtil
                .matchAndSetPropertyByProcessorName(templateProperties, nflowMetadata.getProperties(), NifiPropertyUtil.PROPERTY_MATCH_AND_UPDATE_MODE.NFLOW_DETAILS_MATCH_TEMPLATE);

            //detect template properties that dont match the nflow.properties from the registeredtemplate
            ensureNflowPropertiesExistInTemplate(nflowMetadata, templateProperties);
            nflowMetadata.setProperties(templateProperties);
    ...
    /**
     * If a Template changes the Processor Names the Nflow Properties will no longer be associated to the correct processors
     * This will match any nflow properties to a whose processor name has changed in
     * the template to the template processor/property based upon the template processor type.
     */
    private void ensureNflowPropertiesExistInTemplate(NflowMetadata nflow, List<NifiProperty> templateProperties) {
        ...
        //store the Nflow Properties
        Map<String, String> processorIdProcessorTypeMap = new HashMap<>();
        nflow.getProperties().stream().filter(property -> !processorIdProcessorTypeMap.containsKey(property.getProcessorId())).forEach(property1 -> {
            processorIdProcessorTypeMap.put(property1.getProcessorId(), property1.getProcessorType());
        });

        nflow.getProperties().stream().filter(property -> !templateProcessors.contains(property.getProcessorName())).forEach(property -> {
            //if the property doesn't match the template but the type matches try to merge in the nflow properties overwriting the template ones
    ...
    private NflowManagerTemplate ensureNifiTemplateId(NflowManagerTemplate nflowManagerTemplate) {
        if (nflowManagerTemplate.getNifiTemplateId() == null) {
            String nifiTemplateId = nifiTemplateIdForTemplateName(nflowManagerTemplate.getName());
            nflowManagerTemplate.setNifiTemplateId(nifiTemplateId);
        }
        return nflowManagerTemplate;
    }
    ...
    private RegisteredTemplate saveTemplate(RegisteredTemplate registeredTemplate) {
        ...
        if (template != null && !template.getId().equalsIgnoreCase(registeredTemplate.getId())) {
            ...
            NflowManagerTemplate domain = templateModelTransform.REGISTERED_TEMPLATE_TO_DOMAIN.apply(registeredTemplate);
            ...
            if (orderedTemplateIds != null && !orderedTemplateIds.isEmpty()) {
                IntStream.range(0, orderedTemplateIds.size()).forEach(i -> {
                    ...
                    if (!"NEW".equals(id) && (exclude == null || (exclude != null && !exclude.contains(id)))) {
                        NflowManagerTemplate template = templateProvider.findById(templateProvider.resolveId(id));
                        if (template != null) {
