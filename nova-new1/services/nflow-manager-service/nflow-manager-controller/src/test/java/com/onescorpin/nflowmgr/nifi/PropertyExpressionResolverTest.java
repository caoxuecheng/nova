...
public class PropertyExpressionResolverTest {
    ...
    public void testStrSubstitutor() {
        String template1 = "${metadata.nflowName}_config.xx";
        Map<String, String> map1 = new HashMap<>();
        map1.put("metadata.nflowName", "category.nflow");
        StrSubstitutor ss1 = new StrSubstitutor(map1);
        ss1.setEnableSubstitutionInVariables(true);
        Assert.assertEquals("category.nflow_config.xx", ss1.replace(template1));

        String template2 = "$${${metadata.nflowName}_config.xx}";
        StrSubstitutor ss2 = new StrSubstitutor(map1);
        ss2.setEnableSubstitutionInVariables(true);
        Assert.assertEquals("${category.nflow_config.xx}", ss2.replace(template2));

        String template3 = "${${metadata.nflowName}_config.xx}";
        map1.put("category.nflow_config.xx", "runtime value");
    ...
    public void resolveExpression() {
        final NflowMetadata metadata = new NflowMetadata();
        metadata.setSystemNflowName("mynflow");
        ...
        final NifiProperty prop2 = createProperty("${metadata.systemNflowName}");
        Assert.assertTrue(resolver.resolveExpression(metadata, prop2));
        Assert.assertEquals("mynflow", prop2.getValue());

        // Verify static config
        final NifiProperty prop3 = createProperty(STATIC_KEY, "${metadata.systemNflowName}");
        ...
        final NifiProperty prop6 = createProperty("${metadata.systemNflowName}.${config.test.value}");
        Assert.assertTrue(resolver.resolveExpression(metadata, prop6));
        Assert.assertEquals("mynflow.hello-world", prop6.getValue());

        // Verify multiple variables
        final NifiProperty prop7 = createProperty("$${${metadata.systemNflowName}.${config.test.value}}");
        Assert.assertTrue(resolver.resolveExpression(metadata, prop7));
        Assert.assertEquals("${mynflow.hello-world}", prop7.getValue());

        // Verify multiple variables
        final NifiProperty prop8 = createProperty("${config.${metadata.systemNflowName}.${config.test.value}}");
    ...
    @Test
    public void testNflowMetadataProperties() {
        NflowMetadata metadata = new NflowMetadata();
        metadata.setSystemNflowName("nflowSystemName");
        metadata.setCategory(new NflowCategory());
