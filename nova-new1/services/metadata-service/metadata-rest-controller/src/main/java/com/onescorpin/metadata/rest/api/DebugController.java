package com.onescorpin.metadata.rest.api;
...
import com.onescorpin.metadata.modeshape.support.JcrPath;
...
import com.onescorpin.metadata.modeshape.support.ModeshapeIndexUtil;
...
import com.onescorpin.metadata.rest.model.jcr.JcrIndexDefinition;
import com.onescorpin.metadata.rest.model.jcr.JcrQueryResult;
import com.onescorpin.metadata.rest.model.jcr.JcrQueryResultColumn;
import com.onescorpin.metadata.rest.model.jcr.JcrQueryResultColumnValue;
import com.onescorpin.metadata.rest.model.jcr.JcrQueryResultRow;
...
public class DebugController {
    ...
    /**
     * Allows the caller to update status events for the nflow
     *
     * @param nflowName the name of the nflow
     * @param opIdStr  the operation for the nflow
     * @param stateStr the new state to persist
     * @param status   the status of the operation
     * @return the nflow operation status event
     */
    @POST
    @Path("nflowop/event")
    public String postNflowOperationStatusEvent(@QueryParam("nflow") String nflowName,
                                               @QueryParam("op") String opIdStr,
                                               @QueryParam("state") String stateStr,
                                               @QueryParam("status") @DefaultValue("") String status) {
        // TODO: Is this a nflow ops permission?
        this.accessController.checkPermission(AccessController.SERVICES, MetadataAccessControl.ADMIN_METADATA);
        
        NflowOperation.ID opId = null;
        NflowOperation.State state = NflowOperation.State.valueOf(stateStr.toUpperCase());
        OperationStatus opStatus = new OperationStatus(null,nflowName,null, opId, state, status);
        NflowOperationStatusEvent event = new NflowOperationStatusEvent(opStatus);
    ...
    public List<Metric> exampleMetrics() {
        ...
        NflowExecutedSinceSchedule nflowExecutedSinceSchedule = null;
        try {
            nflowExecutedSinceSchedule = new NflowExecutedSinceSchedule("category", "Nflow", "* * * * * ? *");
            metrics.add(nflowExecutedSinceSchedule);

            WithinSchedule withinSchedule = new com.onescorpin.metadata.api.sla.WithinSchedule("* * * * * ? *", "4 hours");
    ...
    /**
     * returns a new nflow precondition model for debugging
     *
    ...
    @Path("precondition")
    @Produces(MediaType.APPLICATION_JSON)
    public NflowPrecondition examplePrecondition() {
        NflowPrecondition procond = new NflowPrecondition("DependingPrecondition");
        procond.addMetrics("Nflow dependson on execution of another nflow",
                           new NflowExecutedSinceNflow("DependentCategory", "DependentNflow", "ExecutedSinceCategory", "ExecutedSinceNflow"));
        return procond;
    }
