package com.onescorpin.spark.rest.controller;
...
import com.onescorpin.spark.rest.model.JdbcDatasource;
import com.onescorpin.spark.rest.model.RegistrationRequest;
...
import com.onescorpin.spark.shell.SparkShellTransformException;
 ...
 */
@Api(tags = "Nflow Manager - Data Wrangler")
@Component
@Path("/v1/spark/shell")
@SwaggerDefinition(tags = @Tag(name = "Nflow Manager - Data Wrangler", description = "data transformations"))
public class SparkShellProxyController {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Registers a new Spark Shell process with Nova.")
    @ApiResponses({
    ...
    public Response transform(@ApiParam(value = "The request indicates the transformations to apply to the source table and how the user wishes the results to be displayed. Exactly one parent or"
        ...
        if (request.getDatasources() != null && !request.getDatasources().isEmpty()) {
            ...
            final List<com.onescorpin.metadata.api.datasource.Datasource.ID> datasourceIds = metadata.read(
                () -> request.getDatasources().stream()
                    .map(com.onescorpin.metadata.datasource.Datasource::getId)
                    .map(datasourceProvider::resolve)
                    .map(id -> {
                        final com.onescorpin.metadata.api.datasource.Datasource datasource = datasourceProvider.getDatasource(id);
                        if (datasource != null) {
                ...
                () -> datasourceIds.stream()
                    ...
                    .map(datasource -> {
                        if (datasource instanceof com.onescorpin.metadata.api.datasource.UserDatasource) {
                            return (com.onescorpin.metadata.datasource.Datasource) datasourceTransform.toDatasource(datasource, DatasourceModelTransform.Level.ADMIN);
                        } else {
                            throw new BadRequestException("Not a supported datasource: " + datasource.getClass().getSimpleName() + " " + datasource.getId());
                        }
                    })
                    .map(datasource -> {
                        if (datasource instanceof com.onescorpin.metadata.datasource.JdbcDatasource) {
                            return new JdbcDatasource((com.onescorpin.metadata.datasource.JdbcDatasource) datasource);
                        } else {
