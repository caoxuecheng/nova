...
public class DataConfidenceRestController {
    ...
    @Inject
    OpsNflowManagerNflowProvider nflowManagerNflowProvider;
    ...
    public DataConfidenceSummary getDataConfidenceSummary() {
        ...
        return metadataAccess.read(() -> {
            ...
            List<? extends LatestNflowJobExecution> latestCheckDataJobs = nflowManagerNflowProvider.findLatestCheckDataJobs();

            if (latestCheckDataJobs != null) {
                List<CheckDataJob> checkDataJobs = latestCheckDataJobs.stream().map(latestNflowJobExecution -> JobModelTransform.checkDataJob(latestNflowJobExecution)).collect(Collectors.toList());
