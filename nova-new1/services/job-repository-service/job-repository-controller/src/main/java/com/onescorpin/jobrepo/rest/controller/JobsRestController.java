...
import com.onescorpin.jobrepo.query.model.transform.ModelUtils;
...
public class JobsRestController {
     ...
     * Abandon the job associated with the given instance id
     *
     * @param nflowName Full nflow name (including category) for which all jobs are to be abandoned
     * @return Nflow Health status
     */
    @POST
    @Path("/abandon-all/{nflowName}")
    @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_FORM_URLENCODED})
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation("Abandons all jobs for the specified nflow.")
    @ApiResponses(
        @ApiResponse(code = 204, message = "Return no content", response = Void.class)
    )
    public Response abandonAllJobs(@Context HttpServletRequest request, @PathParam("nflowName") String nflowName) {

        this.accessController.checkPermission(AccessController.SERVICES, OperationsAccessControl.ADMIN_OPS);

        metadataAccess.commit(() -> {
            opsNflowManagerNflowProvider.abandonNflowJobs(nflowName);
        });
    ...
    public List<JobStatusCount> findDailyStatusCount(@QueryParam("period") String periodString) {
        ...
        return metadataAccess.read(() -> {
            List<com.onescorpin.metadata.api.jobrepo.job.JobStatusCount> counts = jobExecutionProvider.getJobStatusCountByDateFromNow(period, null);
            if (counts != null) {
    ...
    public List<JobStatusCount> getRunningOrFailedJobCounts() {
        ...
        return metadataAccess.read(() -> {
            List<com.onescorpin.metadata.api.jobrepo.job.JobStatusCount> counts = jobExecutionProvider.getJobStatusCount(jobExecutionProvider.RUNNING_OR_FAILED_FILTER);
            if (counts != null) {
