package com.onescorpin.security.rest.model;
...
public class EntityAccessControl {
    ...
    private List<RoleMembership> nflowRoleMemberships;
    ...
    public List<RoleMembership> getNflowRoleMemberships() {
        if (nflowRoleMemberships == null) {
            nflowRoleMemberships = new ArrayList<>();
        }
        return nflowRoleMemberships;
    }
    
    public void setNflowRoleMemberships(List<RoleMembership> roleMemberships) {
        this.nflowRoleMemberships = roleMemberships;
    }
    ...
    public List<RoleMembershipChange> toNflowRoleMembershipChangeList() {
        List<RoleMembershipChange> membershipChanges = new ArrayList<>();
        for (RoleMembership membership : getNflowRoleMemberships()) {
            RoleMembershipChange roleMembershipChange = new RoleMembershipChange();
            roleMembershipChange.setRoleName(membership.getRole().getSystemName());
            roleMembershipChange.setChange(RoleMembershipChange.ChangeType.REPLACE);
            membership.getGroups().stream().forEach(group -> roleMembershipChange.getGroups().add(group.getSystemName()));
