...
if [ $# -lt 3 ] || [ $# -gt 6 ]; then
    echo "Unknown arguments. Arg1 should be the nifi version, Arg2 should be the nifi home, Arg3 should be the nifi user, Arg4 should be the nifi group. For offline mode pass Arg5 the nova setup folder and Arg6 the -o -or -O option"
    exit 1
fi
...
sed -i "s|nifi.provenance.repository.implementation=org.apache.nifi.provenance.PersistentProvenanceRepository|nifi.provenance.repository.implementation=com.onescorpin.nifi.provenance.repo.NovaPersistentProvenanceEventRepository|" $NIFI_INSTALL_HOME/current/conf/nifi.properties
