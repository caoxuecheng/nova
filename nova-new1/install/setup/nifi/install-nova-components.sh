...
if [ $# -eq 4 ]
then
    echo "The NIFI home folder is $1 and the Nova home folder is $2 using permissions  $NIFI_USER:$NIFI_GROUP"
elif [ $# -eq 6 ] && ([ "$6" = "-o" ] || [ "$6" = "-O" ])
then
    echo "Working in offline mode"
        NOVA_OFFLINE=true
else
    echo "Unknown arguments. The first argument should be the path to the nifi home folder and the second argument should be the nova home. To enable offline mode pass the Nova setup folder and the -o option last "
    exit 1
fi
...
if [ $NOVA_OFFLINE = true ]
then
    NIFI_SETUP_DIR=$NOVA_WORKING_DIR/nifi
else
    NIFI_SETUP_DIR=$NOVA_INSTALL_HOME/setup/nifi
fi

echo -e "\n\n# Set nova nifi configuration file directory path" >> $NIFI_INSTALL_HOME/current/conf/bootstrap.conf
echo -e "java.arg.15=-Dnova.nifi.configPath=$NIFI_INSTALL_HOME/ext-config" >> $NIFI_INSTALL_HOME/current/conf/bootstrap.conf

echo "Installing the nova libraries to the NiFi lib"
...
cp $NIFI_SETUP_DIR/nova-spark-*.jar $NIFI_INSTALL_HOME/data/lib/app
...
then
    ...
    cp $NOVA_INSTALL_HOME/nova-services/lib/mariadb-java-client-*.jar $NIFI_INSTALL_HOME/mysql
fi
...
mkdir $NIFI_INSTALL_HOME/nflow_flowfile_cache/
chown $NIFI_USER:$NIFI_GROUP $NIFI_INSTALL_HOME/nflow_flowfile_cache/
...
sed -i "s|nova.provenance.cache.location=\/opt\/nifi\/nflow-event-statistics.gz|nova.provenance.cache.location=$NIFI_INSTALL_HOME\/nflow-event-statistics.gz|" $NIFI_INSTALL_HOME/ext-config/config.properties
