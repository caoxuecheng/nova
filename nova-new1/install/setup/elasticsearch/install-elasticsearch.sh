...
#########
#  Example 1: ./install-elasticsearch /opt/nova-dev/setup
#  Example 2: ./install-elasticsearch /tmp/offline-install -o
#########
echo "Installing Elasticsearch"
offline=false
SETUP_FOLDER=/opt/nova/setup

if [ $# -eq 0 ]
then
    echo "No setup folder specified. Defaulting to /opt/nova/setup"
elif [ $# -eq 1 ]
then
...
if [ "$retval" == 0 ]
then
        echo "Waiting for 10 seconds for the engine to start up, and then will create Nova indexes in Elasticsearch."
        echo "NOTE: If they already exist, an index_already_exists_exception will be reported. This is OK."
        sleep 10s
        $SETUP_FOLDER/../bin/create-nova-indexes-es.sh localhost 9200 1 1
else
        echo "$SERVICE service did not start within a reasonable time. Please check and start it. Then, execute this script manually before starting Nova."
        echo "This script will create Nova indexes in Elasticsearch."
        echo "NOTE: If they already exist, an index_already_exists_exception will be reported. This is OK."
        echo "$SETUP_FOLDER/../bin/create-nova-indexes-es.sh localhost 9200 1 1"
fi
