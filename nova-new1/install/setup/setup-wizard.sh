...
echo "Welcome to the Nova setup wizard. Lets get started !!!"
echo " "
...
read -p "Enter the nova home folder location, hit Enter for '/opt/nova': " nova_home_folder

if [[ -z "$nova_home_folder" ]]; then
    nova_home_folder=/opt/nova
fi
...
if [ "$install_db" == "y"  ] || [ "$install_db" == "Y" ] ; then
    ...
    while [[ ! $install_db_liquibase =~ $yes_no ]]; do
        read -p "Would you like Nova to manage installing and upgrading the database automatically? Please enter y/n: " install_db_liquibase
    done

    if [ "$install_db_liquibase" == "n"  ] || [ "$install_db_liquibase" == "N" ] ; then
        echo " ";
        echo "OK. Disabling Liquibase in application.properties. Please see the Nova documentation to see how to generate the database scripts"
        sed -i "s|liquibase.enabled=true|liquibase.enabled=false|" $nova_home_folder/nova-services/conf/application.properties
    ...
    while : ; do
        ...
        if [ "$database_type" == "1"  ] ; then
            echo "Creating MySQL database 'nova'"
            ! mysql -h ${hostname} -u "${username}" --password="${password}" -e "create database if not exists nova character set utf8 collate utf8_general_ci;" || break;
        fi
        if [ "$database_type" == "2"  ] ; then
            echo "Creating PostgreSQL database 'nova'"
            ! PGPASSWORD="${password}" createdb -U nova -h ${hostname} -E UTF8 -e nova || break;
        fi
        if [ "$database_type" == "3"  ] ; then
            echo "Creating SQL Server database 'nova'"
            ...
            ! sqlcmd -S ${hostname} -U "${username}" -P "${password}" -Q "CREATE DATABASE nova ${azure_options}" || break;
        fi
    done
fi
echo " ";
...
if [ ! -f $nova_home_folder/users.properties ]; then
    while true; do
    ...
    echo "dladmin=$DLADMIN_PASSWORD_1" >> $nova_home_folder/users.properties
    sed -i "s|#security.auth.file.users=file:\/\/\/opt\/nova\/users.properties|security.auth.file.users=file:\/\/$nova_home_folder\/users.properties|" $nova_home_folder/nova-services/conf/application.properties
    sed -i "s|#security.auth.file.users=file:\/\/\/opt\/nova\/users.properties|security.auth.file.users=file:\/\/$nova_home_folder\/users.properties|" $nova_home_folder/nova-ui/conf/application.properties
    USERS_FILE_CREATED=true

    if [ ! -f $nova_home_folder/groups.properties ]; then
        touch $nova_home_folder/groups.properties
        sed -i "s|#security.auth.file.users=file:\/\/\/opt\/nova\/groups.properties|security.auth.file.users=file:\/\/$nova_home_folder\/groups.properties|" $nova_home_folder/nova-services/conf/application.properties
        sed -i "s|#security.auth.file.users=file:\/\/\/opt\/nova\/groups.properties|security.auth.file.users=file:\/\/$nova_home_folder\/groups.properties|" $nova_home_folder/nova-ui/conf/application.properties
    fi
fi

echo " ";
while [[ ! $java_type =~ ^[1-4]{1}$ ]]; do
    echo "Please choose an option to configure Java for Nova, ActiveMQ, and NiFi"
...
then
    ...
    cd $nova_home_folder/setup
fi

if [ "$java_type" == "1" ] ; then
    echo "Using system Java 8 and remove the JAVA_HOME variable from nova-ui and nova-services"
    ./java/remove-default-nova-java-home.sh $nova_home_folder
elif [ "$java_type" == "2" ] ; then
    if [ $OFFLINE = true ]
    then
        ./java/install-java8.sh $nova_home_folder $CURRENT_DIR -O
    else
        ./java/install-java8.sh $nova_home_folder
    fi

elif [ "$java_type" == "3" ] ; then
    ./java/remove-default-nova-java-home.sh $nova_home_folder
    ./java/change-nova-java-home.sh $java_home $nova_home_folder
fi
...
if [ "$install_es" == "y"  ] || [ "$install_es" == "Y" ] ; then
    ...
    then
        ...
        ./elasticsearch/install-elasticsearch.sh $nova_home_folder/setup $ES_JAVA_HOME
    fi
fi
...
if [ "$install_nifi" == "y"  ] || [ "$install_nifi" == "Y" ] ; then
    ...
    if [ $OFFLINE = true ]
    then
        ./nifi/install-nova-components.sh $nifi_home $nova_home_folder $nifi_user $nifi_group $CURRENT_DIR -O
    else
        ./nifi/install-nova-components.sh $nifi_home $nova_home_folder $nifi_user $nifi_group
    fi
...
if [ "$USERS_FILE_CREATED" ] ; then
    echo ""
    echo ""
    echo "users.properties and groups.properties was created in the $nova_home_folder folder with the dladmin user. For more information on configuring users and groups please see the \"Configure Access Control\" page in the Nova Docs"
fi
echo ""
echo "If this is the first time installing Nova, or you made hive/database related changes, you will need to modify those settings in the $nova_home_folder/nova-services/conf/application.properties file ";
