#!/bin/bash
#########
#  Example 1: ./install-java /opt/nova-dev
#  Example 2: ./install-java /opt/nova-dev /tmp/offline-install -o
#########
...
if [ $# -eq 0 ]
then
    echo "No setup folder specified. Defaulting to nova home to /opt/nova"
elif [ $# -eq 1 ]
then
    echo "The nova home folder is $1 "
    NOVA_INSTALL_HOME=$1
elif [ $# -eq 3 ] && ([ "$3" = "-o" ] || [ "$3" = "-O" ])
then
    ...
    echo "Unknown arguments. The first argument should be the path to the nova home folder. Optional you can pass a second argument to set offline mode. The value is -o or -O "
    exit 1
fi
...
then
    ...
    JAVA_SETUP_DIR=${NOVA_INSTALL_HOME}/setup/java
fi
