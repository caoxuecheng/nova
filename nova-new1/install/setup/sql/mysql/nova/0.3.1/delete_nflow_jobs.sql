USE nova;
DROP PROCEDURE IF EXISTS `delete_nflow_jobs`;
DELIMITER $$
CREATE DEFINER=`root`@`%` PROCEDURE `delete_nflow_jobs`(in category varchar(255), in nflow varchar(255))
BEGIN
DECLARE jobName VARCHAR(255) DEFAULT CONCAT(category,'.',nflow);
DECLARE output VARCHAR(4000) DEFAULT '';
SELECT CONCAT(' STARTING DELETE NFLOW ',jobName,' FROM JOB OPERATIONS TABLES') into output;
