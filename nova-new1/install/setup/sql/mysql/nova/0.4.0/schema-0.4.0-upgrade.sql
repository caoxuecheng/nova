...
THEN
ALTER TABLE `nova`.`BATCH_NIFI_JOB`
...
ALTER TABLE `nova`.`BATCH_NIFI_STEP`
...
DELETE FROM nova.BATCH_NIFI_STEP
...
SELECT event_id,  count(*) FROM nova.BATCH_NIFI_STEP
group by event_id
having count(*) >1) x
...
SELECT STEP_EXECUTION_ID,  count(*) FROM nova.BATCH_NIFI_STEP
...
INSERT INTO `nova`.`GENERATED_KEYS`
(`PK_COLUMN`,
`VALUE_COLUMN`)
...
UPDATE `nova`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(JOB_INSTANCE_ID),0)+1 FROM BATCH_JOB_INSTANCE)
WHERE PK_COLUMN = 'JOB_INSTANCE_ID';

UPDATE `nova`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(JOB_EXECUTION_ID),0)+1 FROM BATCH_JOB_EXECUTION)
WHERE PK_COLUMN = 'JOB_EXECUTION_ID';

UPDATE `nova`.`GENERATED_KEYS` SET VALUE_COLUMN = (SELECT COALESCE(MAX(STEP_EXECUTION_ID),0)+1 FROM BATCH_STEP_EXECUTION)
WHERE PK_COLUMN = 'STEP_EXECUTION_ID';

END IF;


CREATE TABLE IF NOT EXISTS `nova`.`BATCH_JOB_EXECUTION_CTX_VALS` (
...
CREATE TABLE IF NOT EXISTS `nova`.`BATCH_STEP_EXECUTION_CTX_VALS` (
  `JOB_EXECUTION_ID` bigint(20) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `nova`.`NIFI_EVENT` (
  `EVENT_ID` bigint(20) NOT NULL,
  `FLOW_FILE_ID` varchar(45) NOT NULL,
  `FM_NFLOW_NAME` varchar(255) DEFAULT NULL,
  ...
  `NFLOW_PROCESS_GROUP_ID` varchar(45) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `nova`.`NIFI_NFLOW_PROCESSOR_STATS` (
  `FM_NFLOW_NAME` varchar(45) NOT NULL,
  `NIFI_PROCESSOR_ID` varchar(45) NOT NULL,
  `NIFI_NFLOW_PROCESS_GROUP_ID` varchar(45) DEFAULT NULL,
  ...
  PRIMARY KEY (`ID`),
  KEY `FM_NFLOW_NAME` (`FM_NFLOW_NAME`),
...
IF NOT EXISTS(SELECT table_name
            FROM INFORMATION_SCHEMA.COLUMNS
           WHERE table_schema = 'nova'
             AND table_name = 'NFLOW' and COLUMN_NAME ='NFLOW_TYPE')
THEN
ALTER TABLE `nova`.`NFLOW`
ADD COLUMN `NFLOW_TYPE` VARCHAR(45) NULL;
...
CREATE TABLE IF NOT EXISTS `NOVA_VERSION` (
   ID binary(16) NOT NULL,
...
THEN
ALTER TABLE `nova`.`SLA_METRIC_ASSESSMENT`
...
ALTER TABLE `nova`.`BATCH_JOB_EXECUTION_PARAMS`
