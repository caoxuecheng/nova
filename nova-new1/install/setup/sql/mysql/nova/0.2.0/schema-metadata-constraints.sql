...
alter table DATA_OPERATION add constraint FKd6xpyscn8jfghycm3b3sv1wq2 foreign key (producer_id) references NFLOW_DESTINATION (id);
...
alter table NFLOW add constraint UK_j4px0sd8c2k3ycpw6uvqplrr unique (display_name);
alter table NFLOW add constraint UK_m3uusi4w4t57ey2153r1pwu32 unique (name);
alter table NFLOW add constraint FKi6tlfq6nytlrb8429acoo6f3s foreign key (sla_id) references SLA (id);
alter table NFLOW_DESTINATION add constraint FKm91lqsf5q7b3jltbb3h23upqy foreign key (datasource_id) references DATASOURCE (id);
alter table NFLOW_DESTINATION add constraint FKq7fugjfa8oliwli38vf8amdr2 foreign key (nflow_id) references NFLOW (id);
alter table NFLOW_PROPERTIES add constraint FK67wcouxn6tab9gxv2u00gcn43 foreign key (JpaNflow_id) references NFLOW (id);
alter table NFLOW_SOURCE add constraint FKmf5yjhbgipi4ufuxj0wmme6gx foreign key (datasource_id) references DATASOURCE (id);
alter table NFLOW_SOURCE add constraint FK7cu1dl0vf0haaasfvshdsf4g7 foreign key (nflow_id) references NFLOW (id);
alter table NFLOW_SOURCE add constraint FK5c9kcq5r4qunp058v6smsprb1 foreign key (agreement_id) references SLA (id);
...
/* NFLOW MANAGER TABLE FK Constraints */

ALTER TABLE `FM_NFLOW`
