...
ALTER TABLE `nova`.`BATCH_STEP_EXECUTION_CTX_VALS`
ADD CONSTRAINT `BATCH_STEP_EXEC_CTX_VALS_FK1`
  FOREIGN KEY (`STEP_EXECUTION_ID`)
  REFERENCES `nova`.`BATCH_STEP_EXECUTION` (`STEP_EXECUTION_ID`);

ALTER TABLE `nova`.`BATCH_STEP_EXECUTION_CTX_VALS`
ADD CONSTRAINT `BATCH_STEP_EXEC_CTX_VALS_FK2`
  FOREIGN KEY (`JOB_EXECUTION_ID`)
  REFERENCES `nova`.`BATCH_JOB_EXECUTION` (`JOB_EXECUTION_ID`);

ALTER TABLE `nova`.`BATCH_JOB_EXECUTION_CTX_VALS`
...
ADD CONSTRAINT `BATCH_NIFI_JOB_FK1`
  ...
  REFERENCES `nova`.`BATCH_JOB_INSTANCE` (`JOB_INSTANCE_ID`);
...
ALTER TABLE `nova`.`BATCH_JOB_INSTANCE`
ADD INDEX `BATCH_JOB_INSTANCE_FK1_idx` (`NFLOW_ID` ASC);

ALTER TABLE `nova`.`BATCH_JOB_INSTANCE`
ADD CONSTRAINT `BATCH_JOB_INSTANCE_FK1`
  FOREIGN KEY (`NFLOW_ID`)
  REFERENCES `nova`.`NFLOW` (`id`);

ALTER TABLE `nova`.`BATCH_JOB_EXECUTION`
ADD INDEX `BATCH_JOB_EXECUTION_IDX1` (`START_TIME` DESC);

ALTER TABLE `nova`.`NIFI_EVENT`
ADD INDEX `NIFI_EVENT_IDX1` (`FM_NFLOW_NAME` ASC);
