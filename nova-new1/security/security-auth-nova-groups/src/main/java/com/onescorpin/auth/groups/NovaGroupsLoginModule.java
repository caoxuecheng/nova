...
import com.onescorpin.auth.jaas.AbstractLoginModule;
import com.onescorpin.rest.JerseyRestClient;
...
/**
 * Login module which gets Nova groups. This is used to limit user groups to those available in Nova.
 */
public class NovaGroupsLoginModule extends AbstractLoginModule implements LoginModule {

    private static final Logger log = LoggerFactory.getLogger(NovaGroupsLoginModule.class);
    /**
    ...
    protected boolean doLogin() throws Exception {
        ...
        try {
            ...
            log.error("An error occurred while getting Nova Groups username: {}", username, e);
            throw new RuntimeException("An error occurred while getting Nova Groups", e);
        }
    ...
    protected boolean doCommit() throws Exception {
        ...
        Set<Principal> novaGroups = getPrincipals(); //nova groups, e.g. A, B, C
        otherPrincipals.removeIf(principal -> principal instanceof GroupPrincipal && !novaGroups.contains(principal)); //intersection result, e.g. B, C
        return true;
    }
