package com.onescorpin.auth.cache;
...
public class UserCacheAuthConfig {
    ...
    @Value("${security.auth.file.login.check.order:#{T(com.onescorpin.auth.jaas.LoginConfiguration).LOWEST_ORDER}}")
    private int loginCheckOrder;
    
    @Value("${security.auth.file.login.save.order:#{T(com.onescorpin.auth.jaas.LoginConfiguration).HIGHEST_ORDER}}")
