...
public class NovaRestLoginModuleTest {
    ...
    public void test() throws Exception {
        ...
        final CallbackHandler callbackHandler = callbacks -> Arrays.stream(callbacks).forEach(callback -> {
            if (callback instanceof NameCallback) {
                ...
                ((PasswordCallback) callback).setPassword("onescorpin".toCharArray());
            }
        });

        // Mock login module
        final NovaRestLoginModule module = new NovaRestLoginModule() {
            @Nonnull
            @Override
        ...
        options.put(NovaRestLoginModule.REST_CLIENT_CONFIG, new LoginJerseyClientConfig());
