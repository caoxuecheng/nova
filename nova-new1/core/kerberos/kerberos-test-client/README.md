...
## How to Use
java -jar nova-kerberos-test-client-<version>.jar

## Example

~~~~
[root@sandbox opt]# /usr/lib/jvm/java/bin/java -jar /opt/nova-kerberos-test-client-0.4.2-SNAPSHOT.jar

Please enter the list of configuration resources:  /etc/hadoop/conf/core-site.xml,/etc/hadoop/conf/hdfs-site.xml

Please enter the keytab file location: /etc/security/keytabs/nova.headless.keytab

Please enter the principal name: onescorpin@sandbox.hortonworks.com
...
Generating Kerberos ticket for principal: onescorpin@sandbox.hortonworks.com at key tab location: /etc/security/keytabs/onescorpin.headless.keytab
Java config name: /etc/krb5.conf
Loaded from Java config
>>> KdcAccessibility: reset
>>> KdcAccessibility: reset
>>> KeyTabInputStream, readName(): sandbox.hortonworks.com
>>> KeyTabInputStream, readName(): onescorpin
...
>>> KrbAsRep cons in KrbAsReq.getReply onescorpin
