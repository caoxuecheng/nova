package com.onescorpin.cluster;

/*-
 * #%L
 * nova-cluster-manager-core
 * %%
...
public class JGroupsClusterService extends ReceiverAdapter implements ClusterService {
    ...
    @Value("${nova.cluster.jgroupsConfigFile:#{null}}")
    private String jgroupsConfigFile;

    @Value("${nova.cluster.acknowledge:false}")
    private boolean sendAcknowledgementMessage = false;

    @Value("${nova.cluster.pending.acknowledge.expire.minutes:30}")
    private Integer pendingMessageAcknowledgeTime = 30;

    private static final String CLUSTER_NAME = "internal-nova-cluster";
    ...
    public void start() throws Exception {
        if (channel != null) {
            ...
            try {
                ...
                log.error("Unable to find the jgroups cluster configuration file {}.  Nova is not clustered ", jgroupsConfigFile);
            }
        }
    }
    ...
    public void clusterEnabled() {
        if (channel == null) {
            throw new UnsupportedOperationException(" This is not a clustered Nova");
        }
    }
