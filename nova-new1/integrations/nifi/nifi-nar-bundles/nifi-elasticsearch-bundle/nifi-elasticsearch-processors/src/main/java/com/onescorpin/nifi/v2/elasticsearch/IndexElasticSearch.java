package com.onescorpin.nifi.v2.elasticsearch;

/*-
 * #%L
 * onescorpin-nifi-elasticsearch-processors
 * %%
...
import com.onescorpin.hashing.HashingUtil;
...
public class IndexElasticSearch extends AbstractNiFiProcessor {
    ...
    /**
     * Property for Nova category name
     */
    public static final PropertyDescriptor CATEGORY_NAME = new PropertyDescriptor.Builder()
        .name("NovaCategory")
        .description("Nova category system name for data to be indexed")
        .required(true)
    ...
    /**
     * Property for Nova nflow name
     */
    public static final PropertyDescriptor NFLOW_NAME = new PropertyDescriptor.Builder()
        .name("NovaNflow")
        .description("Nova nflow system name for data to be indexed")
        .required(true)
        .defaultValue("${nflow}")
        .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
        .expressionLanguageSupported(true)
    ...
    private boolean sendToElasticSearch(String json,
        ...
        for (int i = 0; i < array.length(); i++) {
            ...
            } else if (StringUtils.isNotEmpty(categoryName) && (StringUtils.isNotEmpty(nflowName))) {
                String hash = HashingUtil.getHashMD5(jsonObj.toString());
                if (StringUtils.isNotEmpty(hash)) {
                    id = categoryName + "::" + nflowName + "::" + hash;
