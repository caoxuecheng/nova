...
import com.onescorpin.nifi.v2.core.metadata.MetadataProviderSelectorService;
...
/**
 * To test with SSL 1. use the nifi-toolkit to generate selfsigned key and trust store tls-toolkit.sh standalone -n 'localhost' -C 'CN=nova, OU=onescorpin' -o . 2. set the application properties to make
 * onescorpin-services run under SSL  using the generated files from #1  (example below) server.ssl.key-store=/Users/sr186054/tools/test-ssl/test/localhost/keystore.jks
...
public class MetadataProviderSelectorServiceTest {
    ...
    private SSLContextService createSslContextService(TestRunner runner) throws InitializationException {
        ...
        runner.setProperty(service, StandardSSLContextService.KEYSTORE.getName(), "/Users/sr186054/tools/test-ssl/test/CN=nova_OU=onescorpin.p12");
    ...
    private MetadataProviderSelectorService createMetadataProviderSelectorService(TestRunner runner, boolean useSsl) throws InitializationException {
        ...
        runner.setProperty(metadataProviderSelectorService, MetadataProviderSelectorService.CLIENT_PASSWORD.getName(), "onescorpin");
    ...
    public void testMetadataService() throws InitializationException {
        ...
        String nflowId = metadataService.getProvider().getNflowId(category, nflow);
        Assert.assertNotNull(nflowId);
