...
public class JmsCleanupEventConsumer implements CleanupEventConsumer {
    ...
    /**
     * Map of nflow category and name to listener
     */
    @Nonnull
     ...
     */
    @Override
    public void addListener(@Nonnull String category, @Nonnull String nflowName, @Nonnull CleanupListener listener) {
        String key = generateKey(category, nflowName);
    ...
    public void receiveEvent(@Nonnull final NflowCleanupTriggerEvent event) {
        LOG.debug("Received JMS message - topic: {}, message: {}", MetadataQueues.CLEANUP_TRIGGER, event);
        LOG.info("Received nflow cleanup trigger event: {}", event);

        String key = generateKey(event.getCategoryName(), event.getNflowName());
    ...
    /**
     * Gets the key for the specified nflow.
     *
     * @param category the category system name
     * @param nflowName the nflow system name
     * @return the map key
