 ...
 */
public class NflowProcessorStatisticsAggregator {


    private static final NflowProcessorStatisticsAggregator instance = new NflowProcessorStatisticsAggregator();

    private NflowProcessorStatisticsAggregator() {

    }

    public static NflowProcessorStatisticsAggregator getInstance() {
        return instance;
    }
    ...
    public void add(GroupedStats stats, ProvenanceEventRecord event, Long eventId) {
        ...
        stats.addDuration(NflowEventStatistics.getInstance().getEventDuration(eventId));
        ...
        if (NflowEventStatistics.getInstance().isEndingNflowFlow(eventId)) {
            stats.addJobsFinished(1L);
            Long jobTime = NflowEventStatistics.getInstance().getNflowFlowFileDuration(event);
            stats.addJobDuration(jobTime);
            if (NflowEventStatistics.getInstance().hasFailures(event)) {
