...
import com.onescorpin.ingest.TableMergeSyncSupport;
...
import static com.onescorpin.nifi.v2.ingest.IngestProperties.NFLOW_PARTITION;
import static com.onescorpin.nifi.v2.ingest.IngestProperties.FIELD_SPECIFICATION;
import static com.onescorpin.nifi.v2.ingest.IngestProperties.PARTITION_SPECIFICATION;
...
import static com.onescorpin.nifi.v2.ingest.IngestProperties.SOURCE_SCHEMA;
import static com.onescorpin.nifi.v2.ingest.IngestProperties.SOURCE_TABLE;
import static com.onescorpin.nifi.v2.ingest.IngestProperties.TARGET_SCHEMA;
import static com.onescorpin.nifi.v2.ingest.IngestProperties.TARGET_TABLE;
...
public abstract class AbstractMergeTable extends AbstractNiFiProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        String nflowPartitionValue = context.getProperty(NFLOW_PARTITION).evaluateAttributeExpressions(flowFile).getValue();
        ...
        logger.info(
            "Merge strategy: " + mergeStrategyValue + " Using Source: " + sourceTable + " Target: " + targetTable + " nflow partition:" + nflowPartitionValue + " partSpec: " + partitionSpecString);
        ...
        try (final Connection conn = getConnection(context)) {
            ...
            if (STRATEGY_DEDUPE_MERGE.equals(mergeStrategyValue)) {
                mergeSupport.doMerge(sourceSchema, sourceTable, targetSchema, targetTable, partitionSpec, nflowPartitionValue, true);
            } else if (STRATEGY_MERGE.equals(mergeStrategyValue)) {
                mergeSupport.doMerge(sourceSchema, sourceTable, targetSchema, targetTable, partitionSpec, nflowPartitionValue, false);
            } else if (STRATEGY_SYNC.equals(mergeStrategyValue)) {
                mergeSupport.doSync(sourceSchema, sourceTable, targetSchema, targetTable, partitionSpec, nflowPartitionValue);
            } else if (STRATEGY_ROLLING_SYNC.equals(mergeStrategyValue)) {
                mergeSupport.doRollingSync(sourceSchema, sourceTable, targetSchema, targetTable, partitionSpec, nflowPartitionValue);
            } else if (STRATEGY_PK_MERGE.equals(mergeStrategyValue)) {
                mergeSupport.doPKMerge(sourceSchema, sourceTable, targetSchema, targetTable, partitionSpec, nflowPartitionValue, columnSpecs);
            } else {
