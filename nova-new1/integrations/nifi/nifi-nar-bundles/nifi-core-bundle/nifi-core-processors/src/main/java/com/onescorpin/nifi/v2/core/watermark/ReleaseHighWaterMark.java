...
@Tags({"release", "high-water", "mark", "onescorpin"})
@CapabilityDescription("Releases the watermark claim associated with a nflow.")
            ...
            try {
                if (context.getProperty(RELEASE_ALL).asBoolean()) {
                    if (mode.equals("COMMIT")) {
                        ff = recorder.commitAllWaterMarks(session, ff, getNflowId(context, ff));
                    } else {
                        ff = recorder.releaseAllWaterMarks(session, ff, getNflowId(context, ff));
                    }
                } else {
                    String waterMarkName = context.getProperty(HIGH_WATER_MARK).evaluateAttributeExpressions(ff).toString();

                    if (mode.equals("COMMIT")) {
                        ff = recorder.commitWaterMark(session, ff, getNflowId(context, ff), waterMarkName);
                    } else {
                        ff = recorder.releaseWaterMark(session, ff, getNflowId(context, ff), waterMarkName);
                    }
                }
