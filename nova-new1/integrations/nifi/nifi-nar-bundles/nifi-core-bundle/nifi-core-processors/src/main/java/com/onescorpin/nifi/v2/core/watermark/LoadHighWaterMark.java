 ...
 *
 */
package com.onescorpin.nifi.v2.core.watermark;
...
import com.onescorpin.nifi.v2.common.NflowIdNotFoundException;
...
@Tags({"load", "high-water", "mark", "onescorpin"})
@CapabilityDescription("Loads and makes active a watermark associated with a nflow.")
public class LoadHighWaterMark extends HighWaterMarkProcessor {
    ...
    protected static final PropertyDescriptor INITIAL_VALUE = new PropertyDescriptor.Builder()
        .name("Initial Value")
        .description("The initial value for the water mark if none currently exists for the nflow.")
        .required(false)
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        if (outputFF != null) {
            ...
            } catch (NflowIdNotFoundException e) {
                // Initialize should find the current nflow ID.  If we are the head of the flow then the nflow metadata 
                // creation may not have completed before we were triggered we will yield so that we can retry.  
                // Otherwise re-throw the exception.
                if (createdFlowfile && outputFF.getAttribute(NFLOW_ID_ATTR) == null) {
                    getLog().debug("ID for nflow was not available yet - yielding");
            ...
            try {
                String nflowId = getNflowId(context, outputFF);

                try {
                    outputFF = recorder.loadWaterMark(session, outputFF, nflowId, waterMark, propName, initialValue);
                } catch (WaterMarkActiveException e) {
                    throw e;
                } catch (Exception e) {
                    getLog().error("Failed to load the current high-water mark: {} for nflow {}", new Object[]{waterMark, nflowId}, e);
    ...
    /* (non-Javadoc)
     * @see com.onescorpin.nifi.v2.common.BaseProcessor#addProperties(java.util.List)
     */
    @Override
    ...
    /* (non-Javadoc)
     * @see com.onescorpin.nifi.v2.common.BaseProcessor#addRelationships(java.util.Set)
     */
    @Override
