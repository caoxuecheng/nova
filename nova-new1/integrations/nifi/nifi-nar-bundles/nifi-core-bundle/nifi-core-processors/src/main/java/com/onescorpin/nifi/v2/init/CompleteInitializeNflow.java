...
@CapabilityDescription("Indicates that a nflow initialization flow has completed; either successfully or a failed")
public class CompleteInitializeNflow extends NflowProcessor {

    protected static final AllowableValue[] MODE_VALUES = new AllowableValue[]{
        new AllowableValue("SUCCESSFUL", "Successful", "The mode indicating nflow initialization was successful."),
        new AllowableValue("FAILURE", "Failure", "The mode indicating nflow initialization failed.")
    };
    ...
    public void onTrigger(ProcessContext context, ProcessSession session) throws ProcessException {
        ...
        if (inputFF != null) {
            ...
            if ("SUCCESSFUL".equals(mode)) {
                getMetadataRecorder().completeNflowInitialization(getNflowId(context, inputFF));
            } else {
                getMetadataRecorder().failNflowInitialization(getNflowId(context, inputFF));
            }
