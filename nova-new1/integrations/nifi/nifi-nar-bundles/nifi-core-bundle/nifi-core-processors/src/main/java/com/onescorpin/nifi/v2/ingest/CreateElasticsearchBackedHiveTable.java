...
import com.onescorpin.nifi.v2.thrift.ExecuteHQLStatement;
...
@Tags({"elasticsearch", "onescorpin", "hive"})
...
public class CreateElasticsearchBackedHiveTable extends ExecuteHQLStatement {
    ...
    public static final PropertyDescriptor FIELD_INDEX_STRING = new PropertyDescriptor.Builder()
        ...
        .build();
    private final static String NOVA_DATA_INDEX_NAME = "nova-data";
    private final static String NOVA_DATA_INDEX_TYPE = "hive-data";
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        final String nflowName = context.getProperty(IngestProperties.NFLOW_NAME).evaluateAttributeExpressions(flowFile).getValue();
        validateStringProperty(NFLOW_NAME.getDisplayName(), nflowName, session, flowFile);

        final String categoryName = context.getProperty(IngestProperties.NFLOW_CATEGORY).evaluateAttributeExpressions(flowFile).getValue();
        validateStringProperty(NFLOW_CATEGORY.getDisplayName(), categoryName, session, flowFile);
        ...
        validateStringProperty(NFLOW_ROOT.getDisplayName(), indexString, session, flowFile);

        List<String> hiveStatements = getHQLStatements(columnSpecs, nodes, nflowRoot, nflowName, categoryName, useWan, autoIndex, idField, jarUrl, indexString);
    ...
    public String generateHQL(String columnsSQL, String nodes, String locationRoot, String nflowName, String categoryName, String useWan, String autoIndex, String idField) {
        // elastic search records for the nova-data index require the nova_schema and nova_table
        columnsSQL = columnsSQL + ", nova_schema string, nova_table string";

        // Construct location
        Path path = Paths.get(locationRoot, categoryName, nflowName, "index" );
        ...
        sb.append("CREATE EXTERNAL TABLE IF NOT EXISTS ")
            ...
            .append(NOVA_DATA_INDEX_NAME)
            .append("/")
            .append(NOVA_DATA_INDEX_TYPE)
            .append("', 'es.nodes' = '")
            .append(nodes)
    ...
    public List<String> getHQLStatements(ColumnSpec[] columnSpecs, String nodes, String locationRoot, String nflowName, String categoryName, String useWan, String autoIndex, String idField,
        ...
        String hql = generateHQL(columnsSQL, nodes, locationRoot, nflowName, categoryName, useWan, autoIndex, idField);
