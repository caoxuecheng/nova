 ...
 */
@CapabilityDescription("Drops the standard nflow tables managed by the Think Big platform.")
@EventDriven
@InputRequirement(InputRequirement.Requirement.INPUT_REQUIRED)
@Tags({"hive", "ddl", "drop", "onescorpin"})
public class DropNflowTables extends AbstractNiFiProcessor {
     ...
     */
    private static final List<PropertyDescriptor> properties = ImmutableList.of(IngestProperties.THRIFT_SERVICE, IngestProperties.NFLOW_CATEGORY, IngestProperties.NFLOW_NAME, TABLE_TYPE,
                                                                                ADDITIONAL_TABLES);
    ...
    public void onTrigger(@Nonnull final ProcessContext context, @Nonnull final ProcessSession session) throws ProcessException {
        ...
        String entity = context.getProperty(IngestProperties.NFLOW_NAME).evaluateAttributeExpressions(flowFile).getValue();
        ...
        String source = context.getProperty(IngestProperties.NFLOW_CATEGORY).evaluateAttributeExpressions(flowFile).getValue();
