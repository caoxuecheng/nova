...
import com.onescorpin.ingest.GetTableDataSupport;
...
import com.onescorpin.nifi.thrift.api.AbstractRowVisitor;
import com.onescorpin.util.ComponentAttributes;
import com.onescorpin.util.JdbcCommon;
...
import static com.onescorpin.nifi.v2.common.CommonProperties.METADATA_SERVICE;
...
@Tags({"onescorpin", "table", "jdbc", "query", "database"})
...
public class GetTableData extends AbstractNiFiProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        final String categoryName = context.getProperty(NFLOW_CATEGORY).evaluateAttributeExpressions(incoming).getValue();
        final String nflowName = context.getProperty(NFLOW_NAME).evaluateAttributeExpressions(incoming).getValue();
        ...
        try (final Connection conn = dbcpService.getConnection()) {
            ...
            //set output format type and avro schema for nflow setup, if available
            outgoing = session.putAttribute(outgoing, "db.table.output.format", outputType);
            String avroSchemaForNflowSetup = (avroSchema != null) ? JdbcCommon.getAvroSchemaForNflowSetup(avroSchema) : EMPTY_STRING;
            outgoing = session.putAttribute(outgoing, "db.table.avro.schema", avroSchemaForNflowSetup);
            ...
            if (nrOfRows.get() == 0L) {
                ...
                if (strategy == LoadStrategy.INCREMENTAL) {
                    ...
                    logger.info("Recorded load status nflow {} date {}", new Object[]{nflowName, newWaterMarkStr});
                }
    ...
    private String getIncrementalWaterMarkValue(FlowFile ff, PropertyValue waterMarkPropName) {
        if (!waterMarkPropName.isSet()) {
            ...
            if (StringUtils.isEmpty(propName)) {
                ...
                // This can happen if the nflow does not have an initial water mark, and the water mark
