...
import com.onescorpin.nifi.v2.ingest.CreateElasticsearchBackedHiveTable;
import com.onescorpin.nifi.v2.ingest.IngestProperties;
...
public class CreateElasticsearchBackedHiveTableTest {
    ...
    public static final String NFLOW = "test_nflow";
    public static final String NFLOW_ROOT = "s3a://hive-bucket/model.db";
    public static final String LOCATION = NFLOW_ROOT + "/" + CATEGORY + "/" + NFLOW + "/index";
    ...
    public void testGetHQLStatements() throws Exception {
        ...
        List<String> statements = table.getHQLStatements(specs, NODES, NFLOW_ROOT, NFLOW, CATEGORY, "true", "true", "", JAR_URL, FIELD_STRING);
        assertEquals("ADD JAR " + JAR_URL, statements.get(0));
        assertEquals("CREATE EXTERNAL TABLE IF NOT EXISTS " + CATEGORY + "." + NFLOW
                     + "_index (`name` string, `phone` string, processing_dttm string, nova_schema string, nova_table string) "
                     + "STORED BY 'org.elasticsearch.hadoop.hive.EsStorageHandler' "
                     + "LOCATION '" + LOCATION + "' "
                     + "TBLPROPERTIES('es.resource' = 'nova-data/hive-data', 'es.nodes' = '"
                     + NODES + "', 'es.nodes.wan.only' = 'true', 'es.index.auto.create' = 'true')", statements.get(1));
    }

    @Test
    public void testGeneratingHQL() throws Exception {
        String hql = table.generateHQL("id int, count int, name string", NODES, NFLOW_ROOT, NFLOW, CATEGORY, "true", "true", "");
        assertEquals("CREATE EXTERNAL TABLE IF NOT EXISTS " + CATEGORY + "." + NFLOW
                     + "_index (id int, count int, name string, nova_schema string, nova_table string) STORED BY "
    ...
    public void CreateTableRequiredProperties() throws Exception {
        ...
        runner.setProperty(CreateElasticsearchBackedHiveTable.NFLOW_ROOT, NFLOW_ROOT);
        runner.setProperty(CreateElasticsearchBackedHiveTable.FIELD_INDEX_STRING, FIELD_STRING);
        runner.enqueue(new byte[0], ImmutableMap.of("metadata.category.systemName", CATEGORY, "metadata.systemNflowName", NFLOW));
        runner.run();
        ...
        inOrder.verify(thriftService.statement).execute("CREATE EXTERNAL TABLE IF NOT EXISTS " + CATEGORY + "." + NFLOW
                                                        + "_index (`name` string, `phone` string, processing_dttm string, nova_schema string, nova_table string) "
                                                        + "STORED BY 'org.elasticsearch.hadoop.hive.EsStorageHandler' "
                                                        + "LOCATION '" + LOCATION + "' "
                                                        + "TBLPROPERTIES('es.resource' = 'nova-data/hive-data', 'es.nodes' = '"
