...
public class RegisterNflowTablesTest {
     ...
     * Test runner
     */
    private final TestRunner runner = TestRunners.newTestRunner(RegisterNflowTables.class);
    ...
    public void testRegisterTables() throws Exception {
        ...
        inOrder.verify(thriftService.statement).close();
        inOrder.verify(thriftService.statement).execute("CREATE EXTERNAL TABLE IF NOT EXISTS `movies`.`artists_nflow` (`id` string, `first_name` string, `last_name` string)   "
                                                        + "PARTITIONED BY (`processing_dttm` string)  ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED AS TEXTFILE "
                                                        + "LOCATION '/model.db/movies/artists/nflow'");
        inOrder.verify(thriftService.statement).close();
        ...
        runner.setProperty(IngestProperties.NFLOW_FORMAT_SPECS, "ROW FORMAT DELIMITED LINES TERMINATED BY '\n' STORED AS TEXTFILE");
    ...
    public void testRegisterTablesWithMissingCategory() {
        ...
        runner.enqueue(new byte[0], ImmutableMap.of("metadata.systemNflowName", "artists"));
        runner.run();
    ...
    /**
     * Verify error for missing nflow name.
     */
    @Test
    public void testRegisterTablesWithMissingNflow() {
        runner.setProperty(IngestProperties.FIELD_SPECIFICATION, "data|string");
    ...
    public void testRegisterTablesWithTableType() throws Exception {
        runner.setProperty(IngestProperties.FIELD_SPECIFICATION, "id|int\nfirst_name|string\nlast_name|string");
        runner.setProperty(RegisterNflowTables.TABLE_TYPE, "MASTER");
        runner.enqueue(new byte[0], ImmutableMap.of("metadata.category.systemName", "movies", "metadata.systemNflowName", "artists"));
        runner.run();
    ...
    public void testRegisterTablesWithConfig() throws Exception {
        ...
        runner.setProperty(RegisterNflowTables.TABLE_TYPE, "ALL");
        runner.enqueue(new byte[0], ImmutableMap
            .of("metadata.category.systemName", "movies", "metadata.systemNflowName", "artists", "hive.ingest.root", "/var/ingest", "hive.profile.root", "/var/profile/",
        ...
        inOrder.verify(thriftService.statement).execute(
            "CREATE EXTERNAL TABLE IF NOT EXISTS `movies`.`artists_nflow` (`id` string, `first_name` string, `last_name` string)   PARTITIONED BY (`processing_dttm` string)  "
            + "ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' STORED AS TEXTFILE LOCATION '/var/ingest/movies/artists/nflow'");
        inOrder.verify(thriftService.statement).close();
