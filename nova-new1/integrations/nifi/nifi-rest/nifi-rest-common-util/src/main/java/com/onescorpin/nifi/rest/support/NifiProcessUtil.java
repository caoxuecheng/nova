...
public class NifiProcessUtil {
     ...
     */
    public static final String CLEANUP_TYPE = "com.onescorpin.nifi.v2.metadata.TriggerCleanup";

    public static final String TRIGGER_NFLOW_TYPE = "com.onescorpin.nifi.v2.metadata.TriggerNflow";
     ...
     * @param name          the nflow system name to match, case-insensitive
    ...
    /**
     * Filters the specified list of process groups for ones matching the specified nflow name, including versioned process groups.
     *
     * @param processGroups the list of process groups to filter
     * @param nflowName      the nflow system name to match, case-insensitive
     * @return the matching process groups
     */
    @Nonnull
    public static Set<ProcessGroupDTO> findProcessGroupsByNflowName(@Nonnull final Collection<ProcessGroupDTO> processGroups, @Nonnull final String nflowName) {
        Pattern pattern = Pattern.compile("^" + Pattern.quote(nflowName) + "( - \\d+)?$", Pattern.CASE_INSENSITIVE);
