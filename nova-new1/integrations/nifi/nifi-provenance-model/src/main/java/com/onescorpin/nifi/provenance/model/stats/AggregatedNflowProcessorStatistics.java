 ...
 */
public class AggregatedNflowProcessorStatistics implements Serializable {
    ...
    public AggregatedNflowProcessorStatistics() {
    }

    public AggregatedNflowProcessorStatistics(String startingProcessorId, String collectionId, Long collectionIntervalMillis) {
    ...
    public void addEventStats(ProvenanceEventRecordDTO event) {
       // processorStats.computeIfAbsent(event.getComponentId(), processorId -> new AggregatedProcessorStatistics(processorId, event.getComponentName(), collectionId)).add(event);
        ...
        if (StringUtils.isBlank(processGroup) && StringUtils.isNotBlank(event.getNflowProcessGroupId())) {
            processGroup = event.getNflowProcessGroupId();
        }
    }
