...
import com.onescorpin.policy.{FieldPoliciesJsonTransformer, FieldPolicyBuilder}
import com.onescorpin.spark.DataSet
import com.onescorpin.spark.dataprofiler.output.OutputRow
import com.onescorpin.spark.dataprofiler.{Profiler, ProfilerConfiguration}
import com.onescorpin.spark.datavalidator.DataValidator
import com.onescorpin.spark.model.AsyncTransformResponse
import com.onescorpin.spark.rest.model.{TransformQueryResult, TransformResponse}
...
abstract class TransformScript(destination: String, policies: Array[FieldPolicy], validator: DataValidator, profiler: Profiler) {
    ...
    private def getPolicyMap(schema: StructType): util.Map[String, com.onescorpin.policy.FieldPolicy] = {
        ...
        schema.fields
            ...
            .map(name => FieldPolicyBuilder.newBuilder().fieldName(name).nflowFieldName(name).build())
