package com.onescorpin.spark.datavalidator.functions;
...
import com.onescorpin.policy.BaseFieldPolicy;
import com.onescorpin.policy.FieldPolicy;
import com.onescorpin.policy.standardization.AcceptsEmptyValues;
import com.onescorpin.policy.standardization.StandardizationPolicy;
import com.onescorpin.policy.validation.NotNullValidator;
import com.onescorpin.policy.validation.ValidationPolicy;
import com.onescorpin.policy.validation.ValidationResult;
import com.onescorpin.spark.datavalidator.CleansedRowResult;
import com.onescorpin.spark.datavalidator.StandardizationAndValidationResult;
import com.onescorpin.spark.util.InvalidFormatException;
...
import static com.onescorpin.spark.datavalidator.StandardDataValidator.VALID_RESULT;
...
public class CleanseAndValidateRow implements Function<Row, CleansedRowResult> {
     ...
     */
    @Nonnull
    private List<String> getPolicyMapNflowFieldNames() {
        return FluentIterable.from(Arrays.asList(policies))
            .filter(new Predicate<FieldPolicy>() {
                ...
                public boolean apply(@Nullable FieldPolicy input) {
                    return (input != null && input.getNflowField() != null);
                }
            })
            .transform(new com.google.common.base.Function<FieldPolicy, String>() {
                ...
                public String apply(@Nullable FieldPolicy input) {
                    assert input != null;
                    return input.getNflowField().toLowerCase();
                }
            })
            .toList();
    }
     ...
     */
    @Nonnull
    private StructType getSchema(@Nonnull final StructField[] nflowFields) {
        final List<StructField> nflowFieldsList = new ArrayList<>(nflowFields.length);
        final List<String> policyMapNflowFieldNames = getPolicyMapNflowFieldNames();

        for (final StructField nflowField : nflowFields) {
            final String lowerFieldName = nflowField.name().toLowerCase();
            if (policyMapNflowFieldNames.contains(lowerFieldName)) {
                if (!nflowField.name().equalsIgnoreCase(PROCESSING_DTTM_COL) && !nflowField.name().equalsIgnoreCase(REJECT_REASON_COL)) {
                    nflowFieldsList.add(nflowField);
                }
            } else {
                log.warn("Nflow table field {} is not present in policy map", lowerFieldName);
            }
        ...
        if (hasProcessingDttm) {
            nflowFieldsList.add(new StructField(PROCESSING_DTTM_COL, DataTypes.StringType, true, Metadata.empty()));
            nflowFieldsList.add(nflowFieldsList.size() - 1, new StructField(REJECT_REASON_COL, DataTypes.StringType, true, Metadata.empty()));
        } else {
            nflowFieldsList.add(new StructField(REJECT_REASON_COL, DataTypes.StringType, true, Metadata.empty()));
        }

        return new StructType(nflowFieldsList.toArray(new StructField[0]));
    }
