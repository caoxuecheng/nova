...
import com.onescorpin.spark.DataSet;
import com.onescorpin.spark.SparkContextService;
...
import com.onescorpin.spark.datavalidator.functions.SumPartitionLevelCounts;
...
import static com.onescorpin.spark.datavalidator.functions.CleanseAndValidateRow.PROCESSING_DTTM_COL;
import static com.onescorpin.spark.datavalidator.functions.CleanseAndValidateRow.REJECT_REASON_COL;
...
public class StandardDataValidator implements DataValidator, Serializable {
    ...
    public void saveValidToTable(@Nonnull final String databaseName, @Nonnull final String sourceTableName, @Nonnull final String targetTableName, @Nonnull final DataValidatorResult result,
        ...
        final StructType nflowTableSchema = scs.toDataSet(hiveContext, HiveUtils.quoteIdentifier(databaseName, sourceTableName)).schema();
        StructType validTableSchema = scs.toDataSet(hiveContext, HiveUtils.quoteIdentifier(databaseName, targetTableName)).schema();
        DataSet validDataFrame = getRows(validResultRDD, ModifiedSchema.getValidTableSchema(nflowTableSchema.fields(), validTableSchema.fields(), result.getPolicies()), hiveContext);
    ...
    private String toSelectFields(FieldPolicy[] policies1) {
        ...
        for (int i = 0; i < policies1.length; i++) {
            if (policies1[i].getField() != null) {
                log.info("policy [{}] name {} nflowName {}", i, policies1[i].getField(), policies1[i].getNflowField());
                String nflowField = StringUtils.defaultIfEmpty(policies1[i].getNflowField(), policies1[i].getField());
                fields.add("`" + nflowField + "` as `" + policies1[i].getField() + "`");
            }
        }
